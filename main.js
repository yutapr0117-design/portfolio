         ...
         * @rules
         * 1. STRICTLY Vanilla JS. No imports from external modules or CDNs.
 ...
 */
        ...
        function tokenize(s) {
            return String(s || "").toLowerCase().split(/[^\w\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF\-]+/).filter(Boolean);
        }

        function slugify(str) {
            const s = String(str || '').trim().toLowerCase();
            return s.replace(/[\s_]+/g, '-').replace(/[^\w\-\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/g, '') || `p-${generateId().slice(0, 8)}`;
        }
        ...
        /**
         * safeFetchJSON — 改善文書b 12.1: Fetch APIラッパーとステータスコードの厳密な検証
         *
         * fetch() は 404/500 でも Promise を resolve するため、
         * AIが生成するハッピーパスのみのコードを保護する非破壊的ラッパー。
         *
         * @param {string} url - fetch先のURL
         * @param {RequestInit} [options={}] - fetch オプション
         * @returns {Promise<any|null>} パース済みJSONデータ、失敗時はnull
         */
        async function safeFetchJSON(url, options = {}) {
            try {
                const response = await fetch(url, options);
                // fetch特有の罠: HTTPエラーでもPromiseはresolveされる
                if (!response.ok) {
                                ...
                                h('ul', { class: 'hero-value-list', 'aria-label': 'Value Points' },
                                    h('li', {},
                                        h('span', {},
                                            ...
                                            '72回の高速イテレーションにより、設計からデプロイまでをAIチームで完遂。'
                                        )
                                    ),
                                    h('li', {},
                                        h('span', {},
