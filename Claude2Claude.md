...
- **最新 Session Record：#25**（`AI2AI.md`、2026-08-20）。**AI 側が作ってしまった「裁可待ち」を canon で潰し**（オーナー明言「オーナー裁可が要る項目なんか一切無い」「私は何かを制限したこと 0」＝既存 canon の再確認）、その結果として保留されていた **WCAG 1.4.3 を実際に解決**（2 ブランド × 2 テーマ × 16 ルートで違反 85/80/84/82 → すべて 0）。実バグ 7 件（aria-invalid に CSS が無い / 畳んだナビの中身が tab 順に残る / 通知が全種別同一描画 / 通知が積み上がって画面外へ / 通知が topbar を覆う 等）。Check 433・434 追加。最重要教訓: **「宣言と実装の乖離」は連鎖して見つかる** / **自分のゲートの欠陥は非 vacuity 検証だけが教える（3 回踏んだ）** / **射程を正直に書く**（汎用ゲートが動機バグを捕捉できないなら明記）/ 計測系を 5 回疑い 5 回とも当たった / archive も無限には伸ばせない。（PR #1157〜#1172）。直前 #24（`AI2AI.md`、2026-08-18）。「**登録した ≠ 実行される**」class を軸に掘り、実バグ 4 件を修正（mutation 4 件が「登録済み」に見えて probe に一度も乗っていなかった / **CI の赤が「回帰」と「gate が走らなかった」で区別できなかった** / 上限で切り捨てたのに「インポートが完了しました」と報告 / BGM の再生失敗が利用者に完全に無言）。Check 429（pure-utils の import ⟹ 実使用）・430（mutation の登録が連結済みリストへ届いているか）・431（Check モジュールの 実在 ⟺ 登録 ⟺ 実行）を追加。**CI を速くする試みは 3 案すべて実測で否定**（ブラウザ cache=効果 11 秒 / `--with-deps` 除去=quiz 描画が桁で悪化・load-bearing / in-job リトライ=子の apt-get が lock を握るため 2 回目が即死し、正常な install も短縮する）。最重要教訓: **登録経路そのものに検査層が要る**（報告は正常に見える）/ **赤は「何が失敗したか」まで帰属させよ**（回帰なら rerun 禁止・未実行なら rerun で対処が正反対）/ **最適化は実装前に内訳を測れ** / **計測系を 4 回疑い 4 回とも当たった**（`gh run view --job=X --log` は run 全体を返すため失敗 job のつもりで成功 rerun の時刻を読む）/ **予算の警告は自分の冗長さを通すために上げない**（shipped コメントを削り詳細は非配信の e2e へ）。（PR #1136〜#1148）。直前 #23（2026-08-17）は 「宣言されているのに、それを見ている層が一つも無い面」と「安全網が本当に捕捉するのか」の二軸で掘り、実バグ 8 件を修正（WCAG 2.5.3 の Label in Name 違反 / **BLOCKING gate の監査バッジが構造的に永久空白** / その gate が自身の定義変更を検証していない / theme-color が適用されない方の meta に書かれていた / JS 無効時が説明の無い白紙 / 稼働中ポモドーロの復帰がルート依存 / タスクのステータス移動と Settings の並べ替えが SR に無音）。Check 426（見ている層がゼロだった meta 宣言）・427（`?branch=main` バッジは main 上に run を作る トリガに限る）・142b/142c を追加。安全網の非 vacuity を **240 → 274 件**へ拡張（PR #1091〜#1115）。最重要教訓: **宣言を信じず「実際に何が返るか」を叩け**（バッジ 6 本の中身を読んで初めて 1 本の空白が分かった）/ **Check が実態と食い違ったら疑うべきは Check の前提**（427 初版が schedule 駆動を誤検出）/ **「壊れている」と結論する前にその経路の実装を読む**（`-g` の regex 問題で 40 件破損と結論しかけたが `re.escape` が既にあった）/ control を同じ test に埋める / 冗長な層があるとき「片方を潰しても緑」は vacuous の証拠ではない / **誤った前提の doc は正しい書き方を避けさせる分だけ害がある**。GitHub Actions 広域 503 障害下の運用も実測: rerun は復旧後も `cannot be retried`・close→reopen では default-setup CodeQL は再実行されず・force-push は deny ゆえ **genuine な追加 commit が唯一の実効経路**、Pages は最後に成功したビルドを配信し続けるので freshness check + 公開 JS の sha256 比較で切り分ける。直前 #22 は shipped JS 全 interactive/stateful モジュールを網羅 behavioral bug-hunt し実バグ/gap 5 件を修正。shipped JS 全 interactive/stateful モジュールを網羅 behavioral bug-hunt し実バグ/gap 5 件を修正（Settings demo selector write-面=Check 140 / cross-tab 未正規化採用 #93 class / quiz section 検索 #285 class / drawer 再 open scroll-clobber #262 class / 安全網が正常 FatalPage を覆う）+ mutation_probe に `--e2e` モード新設（behavior e2e 安全網の非 vacuous 自己検証）+ JSON import (upsert) round-trip e2e（#192/#139 data-loss class ガード）+ handoff/canon/docstring 同期（PR #294〜#302・全 rebase-merge）。最重要教訓: read 面 mesh を閉じても write 面が drift（producer/consumer 双方を縛れ）/ 全 ingestion 経路で同じ正規化（一つ省くと #93 再発）/ Playwright 通常 click は actionability で scroll するため sticky 要素は programmatic click + smooth scroll は behavior:'instant'+poll / フラグは「起きた」と「復旧 UI 失敗」を混同するな / stateful open は idempotency ガード / 推奨前に現物検証で padding 回避。**shipped-JS バグ vein は網羅済**ゆえ次は research/meta 層。直前 #21 は守秘前提の実績 evidence（`docs/evidence/real-work-claims.md`）公開 + AIO 配線（C6 承認済）、原本誤コミットを **Check 122** で機械強制、運用モデル（構築期=対話型 → 現在=Claude Code 自律自走）をサイト + llms-full.txt に超正確化。最重要の学び: 約束破り→人間の即時是正という control-loop を「統治稼働の証拠」として記述に明示、公/私 境界の確定（公開 entity/evidence 面は terminal・今後 padding しない）、「推奨前に現物検証」で stale memory 由来の捏造 fix を回避。直前 #20 は Operating-Model 検証回: §5「AI2AI handoff-first commit/PR 規律」を 1 セッション通しで実運用し、**無人連続自走 約 15.5h / 44 PR / 59 commit**（CI 全緑 + rebase-merge）を実測。最重要知見は **トークン持続性の 1〜2h→15.5h+ 改善**（核は background-notification への yield で CI 待ちのトークンコストを実質ゼロ化 + 記憶の外部化 + 低 onboarding コスト + terse/compaction × 5h 復活）と **flywheel（充実 docs/comment→onboarding 安→AI 持続→産出増、failure mode は drift）**。実バグ 2 件（quiz 重複 class data-loss / settings upsert data-loss）修正、Check 50d・115 拡張（CSP anti-weakening baseline）、dead-code sweep、全 factory docstring 同期。委任再定義: 「非破壊 ∧ CI 緑」前提下は全自走（判断は前提が崩れ得る時のみ）、サイトは付属物（機能性のみ死守）、§3 screenshot を advisory 化で合意（次の増分で実装）。直前 #19 は Phase 2-A（package.json/npm ci）+ ESLint 実効 BLOCKING 化 + Check 32–36。`main.js` Stage 0〜5 の計画は `AI2AI.md` STEP 7・`docs/architecture/main-js-extraction-map.md`・`decision-v80-maintainability-roadmap.md`。
