...
- **最新 Session Record：#32**（`AI2AI.md`、2026-08-23）。#31 でライセンスを機械可読 6 面へ配線したあと、**その被覆を実際に測って完成させた** run。同じ「**走査対象に入らないものは検査されない**」class を 3 段掘り下げた —— **JSON-LD の CreativeWork 10 ノードが無宣言**（binary の XMP/ID3 は ACD-1.0 と言うのに JSON-LD は無言）/ **主要 WebPage ノードが全ルートで 2 つの名前を主張**（同一 @id の property merge。中核宣言と真っ向から矛盾）/ **Article ノードだけ license 無し**（静的 Check にも e2e のルート一覧にも入らない両方の死角）。最後は**週次 probe が私自身のテストの race を捕捉**（手動 RED・probe SURVIVED の環境依存。`dynamic-route` の script 要素がルートを跨いで再利用されるため待ちが前ルートの残骸で充足していた）。**advisory 予算超過を全てゼロに**。**詳細は `docs/incident-artifacts/improvement-notes-claude-v80-phase4-license-surface-coverage-and-probe-caught-race.md`**。
