...
- **最新 Session Record：#27**（`AI2AI.md`、2026-08-21）。レンズを「**その gate は実際に何を見ているのか**」へ統一し、ルート / セレクタ / 走査範囲 / 書かれた根拠 の 4 面で穴を掘り当てた run（PR #1224〜#1234）。実バグ 5 件（WCAG 2.5.3 ゲートが存在しないルートを走査し本物の Settings を一度も検査していなかった＝Check 439 で構造封じ／agentic surface の `filter` が 3 writer 不一致で情報を運べていなかった／0 件経由で projects のリスト意味論が復帰しない／検証エラー marking が直した後も残る＝quiz と Settings を対で修正／quiz 検索語の silent truncation）。**詳細は `docs/incident-artifacts/improvement-notes-claude-v80-phase4-gate-truth-audit.md`**。なお `AI2AI.md` は 1,000 行 BLOCKING が目前で、以後の Session Record は「要点＋improvement-notes へのポインタ」で書く規約。
