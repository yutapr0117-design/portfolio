...
- **最新 Session Record：#31**（`AI2AI.md`、2026-08-23）。#30 で独自ライセンス ACD-1.0 を起草した直後に「**では、その許諾は機械から見えるのか**」を測った run。答えは **7 面すべてゼロ** —— `rel="license"` / JSON-LD の `license` / sitemap / aio-manifest / llms / robots / webmanifest のいずれにも宣言が無く、**ACD-1.0 §6.5 自身が「自動化システムが判定できない許諾は許諾ではない」と述べている**以上、本文が発見できない状態はライセンスが自分の主張を満たしていないことを意味した。6 面を配線し **Check 444** で cross-surface coherence を BLOCKING 強制。派生して基盤の欠陥 2 件 —— **早期警告が構造的に一度も出ない file が 6 つ**（advisory 予算 = hard ceiling。`mutation_samples_archive.py` は BLOCKING まで 1 行なのに無警告で、budget doc の説明文が欠陥そのものを設計として記述していた）→ **Check 443**、および **mutation anchor が一意なまま別ノードへ silent に移動**（Check 362 も 420 も捕捉不能・probe だけが検出）。rotate に rebalance モードを追加。**詳細は `docs/incident-artifacts/improvement-notes-claude-v80-phase4-machine-readable-license-and-inert-warning-layer.md`**。
