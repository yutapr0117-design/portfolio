...
- **最新 Session Record：#28**（`AI2AI.md`、2026-08-21）。前 run の「その gate は実際に何を見ているのか」を **perf 面へ広げた** run（PR #1236〜#1243）。quiz の問題集データ **130,595 bytes（配信 JS+CSS の 15.6%）**が 静的 import + modulepreload で **quiz を開かない訪問者にも毎回取得**されていたのを動的 import へ移し、**同時に Check 120 が測る対象をクリティカルパスへ是正**（「download/parse 負荷の保護」を名乗りながらディスク上の全ファイルを足しており、遅延化で数字が**むしろ増えた**）。**840,932 → 710,337 bytes**、予算は**初のラチェットダウン**。あわせて遅延化が持ち込んだ欠陥 4 件を自分で発見・修正し、**同じ assertion で 3 回 vacuous を踏んだ**経緯も記録した。**詳細は `docs/incident-artifacts/improvement-notes-claude-v80-phase4-critical-path-and-self-correction.md`**。
