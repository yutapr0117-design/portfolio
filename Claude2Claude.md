...
- **最新 Session Record：#29**（`AI2AI.md`、2026-08-21）。レンズを「**機械可読面の契約は誰が見ているのか**」と 「**自分の測定を疑う**」へ寄せた run（PR #1245〜#1251）。実バグ 1 件（英語だけの文 5 箇所に `lang="en"` が無く、既存ゲートは quiz 限定かつ ASCII 判定で絵文字や `→` を含む見出しを見逃していた）。ゲート新設 3 件（動的 import の **MIME** を公開面で検証／agentic surface が敵対的 query でも valid JSON／**Check 440** = コード側から `docs/` への参照が解決する）。**自分が書いた「SW が shell を返す」という誤りも実測で訂正**（このサイトはオフライン対応ではない）。**詳細は `docs/incident-artifacts/improvement-notes-claude-v80-phase4-self-correction-and-machine-facing-gates.md`**。
