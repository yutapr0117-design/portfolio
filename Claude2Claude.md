...
- **最新 Session Record：#36**（`AI2AI.md`、2026-09-10）。**bottleneck register の完成と moderator 通知**の run（PR #1547〜#1568・22 件）。**2026-09-09 に OSI Moderators が両リストへ、「AI が全部または大半を書いたと疑われる投稿は拒否する」「両リストが *AI が自律的に参加しライセンスを起草する proof-of-concept* に使われている」と投稿した**（#119 / 新設 **B14**・逐語は `LICENSES/rounds/`）。**名前は出ていないが記述は当てはまる。名指しの対象かはアーカイブからは決まらないので断定しない。** **発信だけを止めた**（§B.0 の停止バナー / `REVISION-PROTOCOL.md` ゲート 0 / 入口ページ / `HANDOFF.md` §0.5）——**開示は薄めない。****更新前の Code of Conduct を同日中に保存した。** `LICENSES/ACD-OSI-BOTTLENECKS.md` は**14 項目すべてに分析節が付き**、**B1 は「待ち」ではなく提出の産物**、**B3 は「縮められるが答えきれない」**（実測 306 語で 3,888 語に届かない）、**B7 は B10 との一方向の取引**と確定した。機械強制は **Check 460 face (m)**（集計行を表から導出して照合）。不利な事実 3 件（#118〜#120）。**本文は凍結中・ACD-1.0 への返信はゼロ（2026-09-10 14:41 JST 取得時点）。** 詳細は `docs/incident-artifacts/improvement-notes-claude-v80-phase4-bottleneck-register-and-moderation-notice.md`
