...
- **最新 Session Record：#37**（`AI2AI.md`、2026-09-19〜20）。**外部レビューの扱い方と逐語引用の忠実さ**の run（PR #1745〜・21 件）。**この 2 日で最も繰り返し効いた 1 行: 編集はどれも英語としては良くなり、どれも証拠としては壊れる。** **外から届いたものは結論ではなく語彙を運ぶ** ——第三者 AI レポートの前提には事実誤りがあったが（#158）、運んできた 3 語（*crayon* / *vanity* / *contra proferentem*）を corpus 全体に当てると **Licensing Committee 委員長の *unpredictable interpretation*** に到達し（#157・**B1 が二層化**）、**OSI 自身の Proliferation Report の定義**が出て（§1.99）、**1.0 §15.3 の削除理由の誤り**が出た（E25）。**指摘ゼロのレビューは文書についての証拠ではない** ——09-20 の 3 件のうち 2 件は指摘ゼロで、**3 件が読んだ草案にはヘッダの破損が目に見える形で在った**（#167・**reflow で我々が壊し、verify は緑のままだった** → **Check 468f**）。**敵対的な 1 件が名前のある法源を 8 件持ってきた**（#170）——**どれも原典で読んでいない**ので `jurisdictions.md` §9 に「読む対象の列」として置いた（**読む前に引けば、開いていない権威を借りる**）。**条文内の限定は敵対的な読み手にも 5 件見落とされ**、**中核の並列構造は 4 回誤読されている**（#24 / #35 は我々自身・外部 3 人中 2 人 → **E28**）。**逐語引用を初めて全 corpus に当て**、既定の空 cache では 322 中 100 しか照合できないと判明、**269 まで上げて誤引用 6 件**（#165 / #166・**うち 3 件は前日の我々**）。**決め打ちの走査対象を 3 件導出へ**（Check 471 face (f) / 441f・441g / 468）。**「強みとしてだけ」書いていたものを 2 件是正**（#174 特許報復の不在＝**「置けない」であって「置かない方が良い」ではない** / #175 提出パケットの意図的な omission 8 件に**費用が 1 つも無かった**）。**ACD-1.0 / 1.1 は凍結のまま 1 バイトも触っていない**（本文変更はすべて 1.2 草案・E25〜E29）。詳細は `docs/incident-artifacts/improvement-notes-claude-v80-phase4-external-reviews-and-quotation-fidelity.md` **同セッション後半（09-20）に、#170 が置いた 8 件の法源キューを全部原典で当てた** ——**Kneschke は 9 か月前に控訴審で終わっており（HansOLG 5 U 104/24・#177）、反論として引かれた Qimonda は読むと §2.9 の設計の根拠だった（#178）。そして OSD 5 に「役割の非対称」まで広げる読みがリスト上に現れ、我々は狭い読みしか当てていなかった（#179）** ——「読んだ」「1 条ずつ当てた」に 3 つ目が付く: **どの読みで当てたか。**
- **1 つ前の Session Record：#36**（`AI2AI.md`、2026-09-10）。**bottleneck register の完成と moderator 通知**の run（PR #1547〜#1596・50 件）。**2026-09-09 に OSI Moderators が両リストへ、「AI が全部または大半を書いたと疑われる投稿は拒否する」「両リストが *AI が自律的に参加しライセンスを起草する proof-of-concept* に使われている」と投稿した**（#119 / 新設 **B14**・逐語は `LICENSES/rounds/`）。**名前は出ていないが記述は当てはまる。名指しの対象かはアーカイブからは決まらないので断定しない。** **発信だけを止めた**（§B.0 の停止バナー / `REVISION-PROTOCOL.md` ゲート 0 / 入口ページ / `HANDOFF.md` §0.5）——**開示は薄めない。****更新前の Code of Conduct を同日中に保存した。** `LICENSES/ACD-OSI-BOTTLENECKS.md` は**14 項目すべてに分析節が付き**、**B1 は「待ち」ではなく提出の産物**、**B3 は「縮められるが答えきれない」**（実測 306 語で 3,888 語に届かない）、**B7 は B10 との一方向の取引**と確定した。機械強制は **Check 460 face (m)**（集計行を表から導出して照合）。不利な事実 7 件（#118〜#124）と errata 3 件（E14〜E16）。**gap（B10）は承認済み 149 件全数で測定済み**（主題語すべて 0 件）で、**§B.0 も断定から測定へ書き換えた**。**委員長の "four corners" 発言（#124）で B13 に天井**。**本文は凍結中・ACD-1.0 への返信はゼロ（2026-09-10 14:41 JST 取得時点）。** **同日夜、オーナー明言を受けて `LICENSES/ACD-1.1.txt` を新設**（NOT IN FORCE / 7 件の errata を閉じ 4,742 語 77 条・**正味 −154 語**・Check 468 が守る）。**1.0 は 1 文字も触っていない。** **同日夜、オーナーが moderator（Nick Vidal 氏）との off-list の往復を逐語提供** ——**公開通知の 2 日前（09-07）に我々へ直接、ACD-1.0 を名指しして 5 問**が来ており、**5 問すべてに回答済み・その返信への応答はまだ無い**（`rounds/2026-09-07-offlist-…`・**公開アーカイブには無い**）。**前提条件は満たされたが発信可否はオーナーと OSI の側。****#125 は同日解消**（**誤っていたのは我々の公開文** ——4 ファイルを *"the drafting was not directed by me, but the direction of the licence was mine"* へ訂正・**開示は薄めていない**）。**🔴 残る #126**: **通知は steward が 2 日前に否定した当のことを事実として述べ、回答の後に出ている** ——**3 読みあり選べない。私信を根拠にリストで反論しない。** 詳細は `docs/incident-artifacts/improvement-notes-claude-v80-phase4-bottleneck-register-and-moderation-notice.md`
