...
canonical-ref: LICENSES/ACD-1.0.review-responses-clauses.md (第 1 層・条項別) / LICENSES/ACD-1.0.txt (凍結中の本文・唯一の権威)
---

# ACD-1.0 — 想定問答（定型条項・解釈条項への攻撃）

**本書は `ACD-1.0.review-responses-clauses.md` の第 2 層である。** 条項別の第 1 層（§2〜§16 を
条ごとに読む）はそちらに、総論・OSD 逐条・認める弱点・議論の進め方は
`ACD-1.0.review-responses.md` にある。

**なぜ分けたか**: 2026-09-20 に条項別の分冊が 952 行になり、advisory 予算（900 行）を超えた。
**BLOCKING（1,000 行）に当たってから動くのでは遅い**ので、既にあった境界——第 1 層と第 2 層——で
切り出した。**Q 番号は動かしていない**（Q22〜Q33 のまま）。他文書からの参照は
`Q26` のように番号で書かれているので、**番号を振り直せば参照だけが静かに壊れる。**
