...
**⚠ 2026-09-22 追記: この表と下の時間量は、すべて暦日で測っており曜日を持っていない**（`AS-OF.md`「沈黙を読むときの暦」）。**中央値 27.7 時間の物差しに対して週末は 2 倍以上の歪みである。****我々自身の `license-discuss` 2 通は日曜（2026-09-06）に出ている。****ただしこのリストでは週末が応答を止めるとは限らない** ——**Viatrix は土曜に出して約 4 時間で応答**を得た。
...
| **OpenMDW 審査の 9 月継続分**（`license-review` 2026-08-31〜09-14・**34 通 31,893 語**・**2026-09-22 に新規性を機械照合**）| **ほぼ既採掘だった。** #206 は *"plus its September continuation"* と述べながら抽出した 3 件は全部 8 月で、**9 月分に何を探して何が無かったかを記録していなかった**。今回、全 34 通から文単位の鍵を取ってドシエ全文と照合すると、**OFL 先例 / Gay 氏の "separate inquiries" / latex2html / OSAID のパラメータ条項 / IPL-1.0 の broad trigger / "revisit a lot of licenses" はいずれも既出**（多くは 2026-09-14 の掃引で入っていた）。**新規は 1 件** —— Fontana 氏 *"the community surrounding OSI saw **patents as a unique problem** for open source deserving unique (if any) treatment"*（`against.md` #213）。**⚠ 通読ではなく機械照合である。** 長文 3 通（Dolan 1,530 語 / Gay 818 語 / Fontana 801 語）は本文も読んだ | `against.md` #213 |
...
| **両リスト 2026-09（オーナーがアップロード・2026-09-24 に取り込み）** | `license-review` **54 通**（前回 49）/ `license-discuss` **10 通**（前回と同数・新着なし）。**`lists.opensource.org` はこの環境のネットワーク方針で遮断されている（プロキシが 403）ため、オーナーが渡したファイルを使った。** 前回以降の新着は ModelGo 審査の 4 通: **Chestek 氏（2026-09-22 17:45 UTC）** が特許終了条項の非対称（Licensor を訴えた場合だけ終了）を「troubling」とし、新規ライセンスの基準 *"structurally put the licensor in a more favored position"* を**名指しで適用**、「single-grantor」は Apache と区別しないと述べた／**Duan 氏（09-23 05:56）** が認めて、誰を訴えても終了する形へ改めると回答／**McCoy 氏（09-23 14:07）** が「その版を承認対象にするのか」「OSI が見るべきは **canonical な版**」と確認／**Duan 氏（09-23 16:05）** が GitHub を更新、大学サーバの複製は遅れると回答。**⚠ 件数は 5 増えたが、前回の取得時刻より後の日付は 4 通** —— 残る 1 通は古い日付で遅れて載ったもので、前回の複製が無いので特定できない（**1 度の取得は「無いこと」を示さない**の実例）。**ACD への還元**: ACD-1.0 に終了条項は無く（§10.4）争点自体は当たらない。基準 2 の適用例として `review-rules.md` 行 2 に追記し、次版 §16.4 の Steward 留保を説明の要る箇所として記録した。McCoy 氏の「canonical な版を見る」は、我々の凍結 + 安定 URL の運用（`FROZEN.md` / Check 453）と同じ要請である。**ACD-1.0 / 1.1 への返信は依然ゼロ** | `review-rules.md` 行 2 |
| **両リスト 2026-09（再取得・取得時刻 2026-09-21T23:20Z）** | **🔴 オーナーが 2026-09-17 に送ったと報告した ACD-1.1 の Discussion Request が、公開アーカイブに無い。** `license-discuss` は 9 月で 10 通、**最後は 2026-09-17 20:06 UTC の他者メッセージ**（FLA-2.1）であり、**`ACD-1.1` の語は 0 回**。`license-review` の方は **2026-09-21 10:31 UTC まで現行**なので、pipermail 側は動いている。**⚠ 分からないこと**: 理由。moderation 保留 / 送信の失敗 / 別の宛先 —— **我々には区別できず、オーナーは送信控えと bounce の有無で 1 手で分かる**（#91 と同じ形）。**⚠ 確立すること**: **我々の投稿が届かなかった例は、これで 3 件目である**（review の 2 通は #136）。**「アーカイブに我々の投稿が無い」を「送っていない」と読んではならない。** 分析は `against.md` #192 |
| **両リスト 2026-09（前回取得・取得時刻 2026-09-21T17:54Z）** | **ACD-1.0 への返信は依然ゼロ。** `license-review` 49 通 / `license-discuss` 9 通。**我々の名が出る 12 通はすべて他者のスレッド**（OpenMDW / ModelGo）への我々の寄与への言及で、**ACD-1.0 について述べたものは 1 通も無い。** 我々自身の最後の投稿は `license-review` 2026-09-04、`license-discuss` 2026-09-06。**この「ゼロ」は取得時刻についての言明であり、それ以降について何も述べない。** **⚠ 併記**: 少なくとも 2 通の review メッセージが moderation に拒否されており（`against.md` #136）、**「アーカイブに無い」は「送っていない」を意味しない。** **同じ取得で `review-rules.md` §1.103 の資格規則が出た**（#188）|
...
## 2026 年 10 月の OSI 関連イベント —— 何を取りに行くか、何を取りに行かないか

**オーナーが 2026-09-17 に共有**（出所は OSI 公式の Upcoming Events・告知）。
**本申請の引き金はこれらの結果を取り込むことである**（`REVISION-PROTOCOL.md` 分岐 B）。

| 日付 | イベント | ACD にとっての価値 |
| :-- | :-- | :-- |
| 10/7 | Open Source Conference Luxembourg | 低 —— 一般イベント |
| 10/7–9 | Open Source Summit Europe（Prague）| 低〜中 —— ライセンス系セッションがあれば |
| 10/18–20 | **All Things Open（Raleigh）**／**10/20 OSI「State of the Source」track** | **高** —— ライセンス・政策・Open Source AI を扱う。**審査する側が何を論点だと思っているかが出る面** |
| 10/21 | **Opening Convening for Open Source AI（Raleigh・OSI × Duke）** | **最高** —— **OSAID の実装**と研究 agenda。**§1.95 で読んだ checklist の "OSI-approved terms" が実務でどう運用されるかは、ここで動く** |
| 10/26–27 | Open Technology Research Symposium（Barcelona・OSI 共催）| 中 —— 研究側の枠組み |
| 10/27 | Code & Compliance（Brussels）| 中 —— 欧州のコンプライアンス実務。**sui generis database 権は EU の話題である** |
| 10/28–30 | Mozilla Festival（Barcelona）| 低〜中 |

### 取りに行くもの（ACD に効く順）

1. **OSAID の "OSI-approved terms" が、パラメータとデータについて実際にどう運用されるか**
   —— §1.95 の 141 本走査（**該当語 0**）が示す穴が、実務で穴として扱われるのかどうか。
2. **審査側が「AI 向けライセンス」に何を求めているか** —— 我々の gap 3 点（§6 / §8.4 / §9）が
   そこで論じられている語彙と一致するか、ずれているか。**ずれていたら語彙を合わせるのは 1.2 の仕事。**
3. **登壇者・発言者が `license-review` の常連と重なるか** —— 重なるなら
   `reviewer-positions.md` の各人の型に追記する材料になる。

### 取りに行かないもの（先に決めておく）

- **「ACD を宣伝する機会」としては扱わない。** register B2（採用実績）は
  **採用を作るためにライセンスを歪めない**方針の下にあり、**イベントでの働きかけで数を作るのは
  その方針の迂回である。**
- **OSAID 適合を承認の理由として使わない**（理事会が「OSAID 適合は承認とは別」と明言済み）。

### ⚠ 記録の規律

**イベントの内容は一次資料で取る。** 伝聞・要約・SNS の断片を根拠にしない
（`AS-OF.md` の「何の証拠で何の証拠でないか」と同じ線）。**録画・スライド・公式ブログが出たら
`rounds/` へ保存**し、**我々の読みは別の file に置く。**
**そして「イベントで何も出なかった」も結果として書く** —— 取り込むものが無かったなら、
**1.2 は自己発見の改善だけで構成されると提出文に書く**（分岐 B が既に定めている形）。
