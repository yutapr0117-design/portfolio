...
検証していない**ということでもある。さらに厳しく読めば —— **条件を取り去るだけで足りるなら、
0BSD と MIT-0 が既に承認済みで存在する。** したがって ACD-1.0 が別物である根拠は「条件が無いこと」
には**一切かかっておらず**、§6（ML・TDM の積極的許諾と不留保）・§8.4（モデルと出力に届く特許許諾）・
§9（権利の存否から独立）の **3 主題だけにかかっている**。**条件の不在は、この 3 つが争点化するのを
防ぐ土台であって、差別化そのものではない。** これを取り違えると、既存の承認済みライセンスで
足りるものを新しいライセンスとして提出していることになる。
