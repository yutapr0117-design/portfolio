...
| 段 | やること | 落とし穴 |
|---|---|---|
| **① 受領** | **公開アーカイブを一次経路として自分で読む**（オーナーの転送を待たない）。`https://lists.opensource.org/pipermail/license-{review,discuss}_lists.opensource.org/` の索引から月次 `.txt` を**ブラウザ相当の UA** で取り、`LICENSES/rounds/<YYYY-MM-DD>-<venue>[-<thread>]-<sent\|<相手>-received>.txt` へ**無改変**で保存する。整形・要約・翻訳・並べ替えをしない | ここで手を入れると、以降のすべての判断が検証不能になる。**アーカイブの方が転送より速い**（オーナー 2026-09-07）。オーナーが貼った全文も同じ規則で保存するが、**出典はアーカイブを優先する** —— 送信控えは手元にしかなく審査者が確かめられないのに対し、アーカイブは**配送された形**であり読み手が実際に読むものだから |
...
**ゲート 0b（2026-09-15 新設）—— 分岐 (a) を、聞かれる前に決めておく。**
**2026-09-15 の返信で steward は「素直な読みなら現在のやり方を変える」と*先に*述べた**
（`rounds/2026-09-15-offlist-nick-vidal-round3-sent.txt`）。**したがって (a) の中身は、
答えが来る前に決まっていなければならない。これは「待ち」ではなく、いま手を動かせる作業である。**

**素直な読みの下で守るべき境界は 1 つ**: *"an AI has been responsible for **all or most of a
message**"* に当たらないこと ——**つまり、リストへ送る文面の*語*を、人間が書く。**

| 面 | 素直な読みの下で許される | 許されない | 根拠 |
|---|---|---|---|
| **リストへ送る文面の語** | steward が書く | AI が起草して送る | 通知の後半節 |
| **外国語としての推敲・翻訳** | **許される** | — | **通知が *"appropriate"* と明示**（2026-09-09） |
| **legalese の読解・論点の整理** | **許される** | — | 同上（*"organize one's thoughts"*） |
| **測定・数値の算出とその提示の整理** | **許される**（2026-09-15 の返信が実際にこの形で、最終段落で開示している）| 測定を口実に文面ごと起草すること | 通知の区別（*assisting* 対 *independently generating*）|
| **ライセンス本文そのもの** | **既に開示済みで、これは "message" ではない** | 開示を薄めること | §E.1 / B14 の禁止 1 |
| **添付する資料（比較・逐条・測定）** | **message ではない**（process ページは本文でなく添付を求めている）| 添付を口実に本文を長くすること | process ページ |

**⚠ 検証可能性が唯一の弱点である。**「自分で書いた」は外から確かめられない。
**唯一の安価な形は、1 通ごとに何を AI にさせたかを本文で述べること** ——
**2026-09-15 の返信が実際にそうしている**（*"I used AI to organize the measurement data that forms
...
1b. **レビューされたテキストの byte が pin されている**（**分類より先に**）。
    ...
    **「読まれたのが現行草案である」ことは、その時点では仮定だった**（後から byte 一致を確認し、
    結果は合っていた）。**旧版が読まれていれば、分類の全体が別の成果物についての測定になる。**
    **受領時にやること: 提出者から現物を受け取り、`shasum -a 256` で repo の版と照合し、
    一致するなら値を `rounds/` の header に書く。一致しないなら、その版を先に特定する。**
...
10. **送信後、公開アーカイブで実物を確認する**（送信控えではなく、**配信された形**で）。
    ...
    **⚠ このゲートの逆側（2026-09-13・同日に書いた）。** これは**送信後**の確認であって
    **送信前の条件ではない。** 配信は送ってみるまで確かめられないので、
    **「確認できるまで送らない」と読むと循環し、無期限の先延ばしの口実になる。**
    **正しい読みは 3 つ**: (a) **送るのを止めない**、(b) 送った後は**アーカイブに現れるまで
    「提出済み」と記録しない**、(c) **現れなかったら、追撃するのではなく moderator へ照会する**
    （空へ向けて何度も送るのは §3.5 が禁じている）。
    **ゲートを足すことには常にこの費用がある** —— **「まだ通っていない項目がある」は、
    やらない理由として最も使いやすい形をしている。**

## 3.5 何も返ってこないとき

**沈黙は既定の状態であって、異常ではない。** `license-discuss` の 2024-01〜2026-09 を全数調査
すると、**スレッドの 22%、ライセンスを持ち込んだものに限っても 18% が返信ゼロ**である
（`PEER-REVIEW-WATCH.md`「沈黙の基準率」）。ACD が同じ状態になることは十分ありうる。

> **⚠ 2026-09-09 に、ここに書いてあった例が古くなっていた。** 旧文は
> 「ModelGo は 2026 年に 3 度出したフォローアップに**いずれも返信を得ていない**」と述べていた。
> **書いた時点では真だったが、2026-08-28 に McCoy Smith 氏が Licensing Committee として返信し、
> 以後そのスレッドはリスト上で最も活発になった。** 沈黙の例として引くなら、正しい形は
> **「2026-01 / 05 / 07 の 3 通に返信が付かず、2026-08-28 に初めて付いた」**
> —— **沈黙は 7 か月続き、そして終わった。**
...
> **2026-09-17 オーナー: 移動の時期が決まった。** 「**返信来ても来なくても、来月のカンファレンス
> 結果を取り込んで、本申請に出したい**」。**したがって引き金は返信でも日数でもなく、
> 来月のカンファレンスである。** 併せて「**Nick さんには必要事項は返答済み**」——
> moderator の 5 問への回答は 2026-09-07 に送られており、前提条件は満たされている。
>
> **これが作業の優先順位を決める**: **(i) カンファレンスで出たものを取り込む先は
> `ACD-1.2-DRAFT.txt` と `ACD-1.2-CHANGELIST.md` である**（1.1 は凍結済み・Check 453 が pin）。
> **(ii) 出す版は 1.2 になる**ので、`ACD-1.1-CHANGELIST.md` §0.13 と `submission.md` §B.0 の
> **確定手順 5 手**を 1.2 について実行する（版に紐づく Check の付け替え・§4c の数値の再導出・
> XML の生成・FREEZE-DATA への追加・適用版の判断）。**(iii) `license-review` へ送る本文は
> §B.0 のままでは送れない**（AI 起草である旨のバナーが立っている）——**語はオーナーが書く。**
> **(iv) 下の表は、移動の条件ではなく移動前に真にしておく項目として、そのまま生きている。**

**沈黙は同意でも却下でもない**（§3.5）。だが**無期限の保留の理由にもならない**。

**根拠は 2026-09-09 に差し替えた。** 旧文は「ModelGo は 3 回のフォローアップにいずれも返信を
得ていない」を根拠にしていたが、**それは後に返信を得た**（§3.5 の訂正）。**結論は変わらないが、
理由はより強いものになる** —— 同じ記録が示すのは「返信は来ないかもしれない」ではなく
**「返信までの間隔は月単位で、こちらの制御下にない」**である。ModelGo の場合は
**最後の未応答フォローアップから 7 か月、初回提出から 19 か月**だった。
**待ち時間が制御外である以上、移動の引き金を「返信」に置けない**という結論は同じである。
...
| 3 | 自己申告の件数がすべて実測と一致 | **Check 460**（機械強制。面は増えるので**数も範囲もここに書かない** —— 権威は `checks_license_self_reporting.py` の docstring。2026-09-09 に「面 a〜k」と書いてあったが実体は (l) まで在った）|
...
**上の表が全部真でも「出すべきだ」とは言えない。**（2026-09-09 まで「**7 つとも**」と書いてあったが、表は 1 / 2 / 3 / 4 / 5 / 6 / 7 / 7b / 8 の **9 行**である。行を足したときに数を直さなかった —— この文書自身が §3 のゲート 3 で禁じている当のことである。**数を書かずに「上の表」と言う。**） ここが言えるのは「**出すなら、こちら側の準備で
欠けているものは無い**」までである。踏み切りの判断はオーナーが行う。

**2026-09-11・オーナーが経路を 2 本あると述べた。** 原文: *「私の当初の計画は、ディスカッションで
受けた指摘全てを取り込み済みの改善版を OSI 申請に出すことでした。ただ……ディスカッションの
アーカイブの数が少ないから、半月ほど様子見してディスカッション結果を得られないようなら、
私たちで改善進めた次の版を本申請に出すことも考慮に入れてるよ。道は一つでは無いよ。」*

**これは上の表を置き換えない —— 表は「こちら側で言えることを言い切ったか」を測るもので、
オーナーの発言は「いつ判断するか」と「何を持って行くか」を述べている。** 3 つが加わる。

1. **観測の目安がついた。** 半月ほど（発言日から数えるとおよそ **2026-09-26** 前後）。
   **これは引き金ではない。** 日付が来ることは条件ではなく、**判断はオーナーが行う**（§3.6 冒頭）。
   我々の側の仕事は、その日に**準備の不足が理由で選べない経路が無い**ようにしておくことである。
2. **「改善版」の中身が経路によって違う。**
...
3. **分岐 B の方が、こちら側の準備は多い。** 送るのが 1.0 ではなく次版になるため、
   **送る前に版を確定させて凍結する**必要がある（§2）。確定の手順と、その日に CI が
   止まる場所は先に書いてある（`ACD-1.1-CHANGELIST.md` §0.13 と `submission.md` の確定手順）。

**同日の補足で、オーナーが判断の作られ方を述べた。**（**狭めたのではない** —— 本人明言「狭めたつもりは無いよ。それに、二択では無いよ」。） 原文: *「もちろん、ディスカッション結果を得られるのが
ベスト。ディスカッション結果が来そうにない目安が半月くらい。ただし、私たちが自信を持って出せる
改善版が出来るまでは本申請に出す気は無いよ。本申請に出すのは、二つのアーカイブを色々掘ったり、
我々で色々調査し、OSI 承認を受けられると判断した時。むやみやたらにディスカッションから手を引き、
本申請に出すような無謀はしないよ。」*

**半月は「議論が来そうにない」の目安であって、提出の引き金ではない。** そして
**これは 2 つの条件でも二択でもなく、1 本の順序である**（オーナー明言）:

> **B（両アーカイブを掘り、我々で調査する）を行うことで、A（自信を持って出せる改善版）に至る。**

**A は B の副産物であって、別立ての要件ではない。** 上の表（**欠落の不在**）はその途中の
測定であり、到達点ではない ——「欠けは無い」と「出せる」は同じではないが、
**その差を埋めるのが B である。** **したがって「待つ」時間に何をするかは決まっている** ——
**掘る。掘った結果が版になる。**

**そして分岐 B は「discuss から手を引く」ことではない。** 議論が届くのが最善であることは
変わらず、**review を足すのであって discuss をやめるのではない**（オーナー明言）。

**この条件が、我々の作業の優先順位をそのまま決めている。** 最も価値があるのは「出せる」と
言えるようにする作業 —— **(i) 両アーカイブから、実際に承認された／却下された理由を掘ること**、
...
6. **返信は、スレッドの中から返す。** 新規メールを list 宛に書くのではなく、**受け取った
   list からのメールに返信する** —— そうすれば `In-Reply-To` と `References` が自動で載り、
   件名も `Re: <元の件名>` のまま残る。**件名を変えると、ヘッダが正しくても多くの reader で
   別スレッドとして表示される。** 理由は測ってある: **`license-review` へ送った 3 通は
   いずれもヘッダを持たず**（`against.md` #90）、**`license-discuss` の 1 通目は逆に、
   list に無いメッセージへの返信として立っていた**（#121）。**2026-09-11 の実測では、
   2026-09-06 04:47 に送った他者の話題への実質的な回答が、件名を変えた単独スレッドとして立ち
   返信ゼロだった一方、同じ週・同じ話題の元スレッドは 4 通の返信（Perens 氏 2 通・Šuklje 氏）を
   得ている。** **これは因果を示さない**（同じくヘッダを持たない moderator 通知には返信が付いた）。
   **示すのは、こちらの側だけで直せて費用がゼロの差が実在することである。**
