...
#### 追記 1 —— LLM 起草の開示（**2026-09-06 に §B §5 へ折り込み済み**）

Q10 は「隠す選択肢は存在しない」と述べているのに、**実際に送る文面にはその開示が無かった**。
本書と §B が食い違っている状態だったので、「送信済みなら follow-up で、未送信なら §5 に足す」と
書いてあった。

**§B は送信されておらず（`license-discuss` へ出したのは別物で、`license-review` へは未提出。
Check 458 が単一ソースと各 file の一致を強制する）、条件は「未送信」で確定していた。
2026-09-06 に §5 へ折り込んだ**（`against.md` #66）。以下の英文は、折り込んだ文の
**より長い版**として残す —— 短縮版が §5 に、全文が `submission.md` §E.1 にある。

**この節自体が、同じ class の 2 例目だった。** §E の但し書きを直したとき、**同じ指示を出している
本書を掃引しなかった**ので、片方を直したあともう片方が「送る文面にその開示が無い」と主張し
続けていた（#71）。**状態を条件にした指示は、条件が解けた瞬間に全部まとめて更新する。**
...
| 提出形式（gap の説明・最近似の承認済みライセンスとの比較・法的レビューの有無） | 満たしている。`submission-reference.md` §1 / §2 / §5 |
...
**もう 1 つ、2026-09-19 に足した限（`license-discuss` 2022-12-13・Bradley M. Kuhn 氏）。**
氏は別のライセンスを vanity 型と呼び、その理由を**名前の 2 つの限**で述べている ——
*"the name is **either designed to promote the name of the org, or it's designed to make it
seem like it's the only appropriate license for work on the general topic**"*。

**前半は当たらない**（"Autonomous Commons" は組織名でも人名でもない）。**後半は当たりうる。**
**"Autonomous" を冠した名前は「自律的なシステムのための唯一の適切なライセンス」と読まれうる**し、
**この異議は本文を開く前に起きる**ので、§16.3 の *"It is not specific to any project, person,
organisation, jurisdiction, or field of endeavour."* は**答えではあるが、届くのが遅い。**

**そして同じスレッドは、この異議が名称変更で解けることも示している** ——氏は改名後に
*"It's reasonably clear now that this is just a license published by a single organization"* と
評価を変えている。**したがって「改名」は実在する選択肢である。1.2 はまだ凍結も提出もされていない。**

**それでも改名しない。理由を 3 つ、費用の側から述べる。**

1. **名前が指す先を動かさないことが、この instrument が自分に課している規律そのものである**
   （§16.4）。**1.0 と 1.1 は既に凍結され、1.0 は 2026-08-26 に `license-discuss` へ投稿されて
   公開アーカイブに残っている。** 改名は、その記録と次版のあいだの接続を切る。
2. **"Commons" は分野名ではなく、この instrument が実際にしていること**（公有へ献呈する）**の語**であり、
   **"Autonomous" は著作物の*状態*を述べている**（継続的な許可を要しない）——Q14。
   **Kuhn 氏の後半の限は「分野の総称を名乗るもの」に向いており、我々の名は分野名ではない。**
3. **改名には、改名でしか解けない異議が要る。** 現に述べられている異議は**読み**であって、
   **本文 §16.3 と Q14 で解ける。**

**⚠ この判断が誤りうる形**: **異議が「読み」ではなく「印象」である場合、読み合わせは届かない。**
**Kuhn 氏の例で実際に効いたのは説明ではなく改名だった。** ——**だから記録として残す**
（`against.md` #161・`review-labels.md` §1.99）。**次に名称が実際に問われたら、
この 3 つの費用を再測して、そのとき決める。**
...
> *"Parameters shall be made available under **OSI-approved terms**."*

**帰結は我々に不利である。** weights / parameters / data information —— まさに ACD-1.0 の §6・§7・§9
が書かれた対象 —— を ACD-1.0 で公開している AI システムは、**ACD-1.0 が承認されるまで OSAID
準拠になれない**。つまり本 instrument は、**自分が書かれた当の用途において、この分野が収束しつつ
ある唯一の枠組みでは使えない**。`against.md` #62 に不利な事実として記録した。

**これを承認の理由として使わない。** 「承認されないと我々の用途が成立しない」は**我々の都合**で
あって、OSD を満たすかどうかとは無関係である。ここで言えるのは、**この instrument にとって承認が
装飾ではなく前提条件だ**という事実までで、それは提出の動機の説明にはなっても、審査の判断材料には
ならない。

**意図的に解かないまま残す点**: OSAID は 2 要素に **"terms"**、1 要素に **"licenses"** と書き分けて
いる。**この非対称が何を意味するのかを、我々は決めない** —— 「terms」が承認済みライセンス以外を
含みうるのかは、我々が答えを持たない解釈問題であり、都合よく読める側に読むのが最も危険な場面
である。`AS-OF.md` に読んだ日付とともに記録した。

### Q32e. moderator に AI の関与を問われたら（2026-09-07 に実際に問われ、答えた）

**これは想定問答ではない。** 2026-09-07、OSI の license 系メーリングリストの moderator である
Nick Vidal 氏が、**ACD-1.0 を名指しして 5 問**を steward へ直接送った（**off-list**。逐語は
`rounds/2026-09-07-offlist-nick-vidal-moderator-exchange.txt`・**公開アーカイブには存在しない**）。
**2 日後の 2026-09-09 に、同趣旨の通知が両リストへ公開された**（`against.md` #119 / register B14）。

**問われた 5 点と、答えた内容**

| 問い | 答え |
|---|---|
| list への投稿は AI が起草／実質的に生成しているか | **Yes。初稿は概ね AI。本人が読み、適切と判断すればそのまま送り、必要なら小さく直して送る**（英語は母語ではないが読み書きでき、翻訳補助を使うこともある）|
| 自律システムが直接投稿・応答しているか | **No。毎通、本人が確認し、本人が送っている** |
| ACD-1.0 は全部／実質的に AI が起草したか | **起草そのものは AI。方向の決定には本人が関与した** |
| OSI の審査過程での自律参加を試すことが目的の一つか | **No。目的はライセンスの承認である** |
| 自分の名前で出した議論と本文を、読み・理解し・責任を負うか | **Yes。読み、理解しており、すべてについて責任を負う用意がある** |

**末尾の指示**: *"Please reply directly to this message **before making further submissions
related to ACD-1.0**."* ——**返信済み。2026-09-10 時点でその返信への応答は無い。**

**この節が establish しないこと**

- **発信を再開してよいこと。** **満たされたのは Nick 氏が述べた前提条件だけ**であり、
