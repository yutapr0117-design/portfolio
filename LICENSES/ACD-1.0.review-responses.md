...
**なぜこう答えるか**: 「dedication は各国法で効かないことがある」は**正しい指摘**であり、否定してはならない。答えは「**§3 の成否に関わらず**、独立に作動するライセンス条項が同じページにある」であり、これは実際にそう起草されている（§2.4 / §4.4）。最後の譲歩（§4 だけ見てくれてよい）は、審査の焦点を**確実に評価可能な部分**へ寄せる実利がある。
...
**`LICENSES/ACD-1.0.faq.md`**（使う側 29 問 / プロセス 5 問 / **リポジトリ逆引き表**）。
...
| 誰が steward か | **Dedicator（適用者）本人**。テキストを書いたのは AI だが、適用し・提出し・責任を負うのは人間である。1.0 のテキストは動かない ——**担保しているのは 1 つではない**: 識別子が 1 つの固定テキストを指し続けるのは §16.4 の目的条項、我々が改訂しないのは `FROZEN.md` と **Check 453** の sha256 pin（**条文の効果ではない**）。改訂は 1.1 以降の別バージョンとして行う。**後継識別子は §16.4 が名指ししていない**ため文言上は守られていない（**E14**） |
...
5. **沈黙も返答である。** 数日で反応が無いのは普通で、催促しない。**決定までの時間は「初回投稿から 60 日」ではなく、最終提出から 2 か月の審査サイクル + 理事会の会合日程**である（**2026-09-10 補足**: 公開規則は Decision Date を「初回提出から 60 日、改訂版は提出から 30 日・ただし初回から 60 日を下回らない」と定める。**改訂は全部やり直しではない。** 実務の説明（McCoy 氏）と公開規則の両方を記録している ——`AS-OF.md`）（McCoy Smith 氏・2026-08-28・`AS-OF.md`）。**改訂のたびに時計が戻る。**
