...
| The text is pure ASCII | byte scan for values > 131 | **0** non-ASCII bytes |
...
| The adverse list is numbered without gaps or duplicates, and every cross-reference between the dossier documents resolves | count `#N` entries; check each `#N` and `EN` citation against the lists | **131 entries, 1–131, no gaps; all citations resolve** |
