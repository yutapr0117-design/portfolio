...
| The text is pure ASCII | byte scan for values > 134 | **0** non-ASCII bytes |
...
| The adverse list is numbered without gaps or duplicates, and every cross-reference between the dossier documents resolves | count `#N` entries; check each `#N` and `EN` citation against the lists | **134 entries, 1–134, no gaps; all citations resolve** |
