...
| The text is pure ASCII | byte scan for values > 130 | **0** non-ASCII bytes |
...
| **The sentence-length profile sits inside the range of approved licences, except on one measure** | split the text at `. ; :` before a capital, count whitespace tokens per sentence, and run the same instrument over the SPDX `licenseText` of Apache-2.0, MPL-2.0, EPL-2.0, OSL-3.0 and CDDL-1.0 | mean **42.9** (EPL-2.0 is 45.3; Apache-2.0 is 36.7), longest sentence **223** words (EPL-2.0 has 328), but **34.2% of sentences exceed 45 words — the highest of the six**. Stated because it is the one measure on which we are worst (`review-precedents.md` §1.70a). **Short sentences are not clarity**: this measures a proxy, and errors specific to a non-native drafter do not appear in it at all |
...
| The adverse list is numbered without gaps or duplicates, and every cross-reference between the dossier documents resolves | count `#N` entries; check each `#N` and `EN` citation against the lists | **130 entries, 1–130, no gaps; all citations resolve** |
