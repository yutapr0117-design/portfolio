...
## 0.9 外部レビューを本文に適用した（2026-09-14）—— 2 件。そして初めて語数が増えた
...
## 0.10 E17 を反映した（2026-09-14・オーナー裁可）—— 見積もりより安かった
...
**語数 4,919 → 4,949（+30）。** **⚠ §0.9 で「『明確さのためなら足してよい』を繰り返し使わない」と
...
|---|---|---|---|
| E1 | §16.1 | 推奨 notice の SPDX 識別子表記 | **直さないと決めた**（§0.7・**承認されれば消える不一致に、恒久のテキストで回避策を書かない**）|
...
| E6 | §8.4 | §8.1 の「Work に含まれる主題に起因する侵害に限る」proviso を意図的に落としている。意図は正しいが、**落としたことが読み取れない** | **狭めないと決めた**（§0.7）**+ 読み取れない方は草案で閉じた** ——§8.4 に「この proviso は意図的に不在である」と明記した |
