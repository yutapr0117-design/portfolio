...
> **⚠ 2026-09-10 追記 —— 発信は止まっている。** タイトルは「申請可能」だが、**いま送れる状態では**
> **ない。** 同日、OSI Moderators が両リストへ **「AI が全部または大半を書いたと疑われる投稿は
> 拒否する」**と投稿した（`against.md` #119 / `ACD-OSI-BOTTLENECKS.md` **B14**・逐語は `rounds/`）。
...
凍結後の敵対的読解で **194 件の不利な事実**と **30 件の errata**（うち §16.1 は本文が読み手に
...
4. **先例が 6 年間くり返されていない（2026-09-06 に実測）。** 公開トラッカー 252 件をこの類型で
   検索すると、承認は **The Unlicense（2020）の 1 件のみ**で（**legacy 承認として*依頼*されたが、OSI 自身の一覧での分類は `special-purpose` であり、理事会がどの基準で承認したかは読めない** ——`board-decisions.md` §1a）、
