...
> **⚠ 発信の状態はここに書かない。単一ソースは `FROZEN.md` の `POSTING-STATUS` marker である。**
> **2026-09-22 実測では `active`** ——**2026-09-17 に再開しており**（`git log` の
> 「発信が再開した」commit）、**この段落は 2026-09-10 から 5 日間、再開後も「止まっている」
...
凍結後の敵対的読解で **249 件の不利な事実**と **34 件の errata**（うち §16.1 は本文が読み手に
