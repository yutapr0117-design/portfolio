...
## 現況（2026-09-24 夕・**ここだけ読めば次の手が決まる**）

**M1〜M12 は是正済み**、**M13〜M15 は未拾得**（1.2 の記述子を作るときの注意で、いま直す場所が無い）。
下の §1 は発見時点の記述のまま残してある（過去形の記録。数字や行番号は発見時のもの）。
**状態は実物で確かめた**（grep の対象を右列に書いた。再確認するときはこの列を当て直せばよい）。

| # | 状態 | 是正した commit | いま確かめる方法 |
| :-- | :-- | :-- | :-- |
| M1 | ✅ 是正済み | `30a952b8` | `grep -c by-nc-nd sitemap.xml` → 0。**再発は Check 444h** が止める |
| M2 | ✅ 是正済み | `30a952b8` ほか | `REUSE.toml` のコメントが CRITICAL で止まることを述べ、`against.md` #108 が「承認で解消するのは命名の 2 行だけ」と書く |
| M3 | ✅ 是正済み | 同 PR 群 | 1.2 草案に `posting is paused` が無い。**再発は Check 467c** |
| M4 | 📝 記録済み（凍結ゆえ本文は不変） | — | `ACD-1.1-SELF-AUDIT.md` §3n |
| M5 | ✅ 是正済み | `28b79b8b` | `ACD-1.1-CHANGELIST.md` §0.5 の対応表 |
| M6 | ✅ 是正済み | `11d2f9aa` ほか | `REVIEWERS.md` が「frozen 時点で 21 件中 19 件・その後も増えている」と書く |
| M7 | ✅ 是正済み | `97514a9b` | `errata.md` が `33/33` |
| M8 | ✅ 是正済み | `30a952b8` | manifest に `spdx_listed` / `osi_approved`。**記述子との一致は Check 451c** |
| M9 | ✅ 是正済み | `30a952b8` / `5a047b2f` | llms 4 面・`LICENSE`・manifest・XMP・ID3 が「商標と人名は与えない（§11.1）」まで述べる |
| M10 | ✅ 是正済み | `30a952b8` | manifest `applies_to` と `LICENSE` が `LICENSES/rounds/` を除く |
| M11 | ✅ 是正済み | `5a047b2f` | WebP に `xmpRights:Marked` が無い（「省略 = 不明」を選んだ） |
| M12 | ✅ 是正済み | `5a047b2f` | MP3 の `WCOP` が `LICENSES/ACD-1.0.txt` の URL |
| M13 | ⏳ 未拾得 | — | **1.2 の記述子を作るとき**に `textRedistribution` の名前と極性を決める（1.1 記述子は凍結中） |
| M14 | ⏳ 未拾得 | — | 同上 ——`osiApproved` / `spdxListed` は本文の事実ではないことを `$comment` の射程に書く |
| M15 | ⏳ 未拾得 | — | 同上 ——1.2 の §15.5 / §15.6 が 1.0 では別の条だったことを、記述子の `clause` で取り違えない |

**M13〜M15 の行き先**: `ACD-1.2-CHANGELIST.md`（記述子の作成手順の節）。**まだ入っていない**
（2026-09-24 に `grep -n "textRedistribution\|M13\|M14\|M15" LICENSES/ACD-1.2-CHANGELIST.md` で 0 件を確認）。
拾うのは CHANGELIST を担当するセッションである（件数の同期が 2 か所で走らないように、この文書からは書き込まない）。

### 同じ日に見つけた、この文書の範囲外の未決 1 件

`OSI-APPROVAL-PLAN.md`（2026-09-24 新設）は、条文の中身と名称（B6）を「オーナーが決める」とし、§6 を
「オーナーの判断を待っているもの」の表にしている。一方 `ACD-1.2-CHANGELIST.md` の採用候補の節は
「オーナー裁可待ちではなく、設計判断なので献策として置く」と述べ、`CLAUDE.md` §7 は「裁可待ち」という
作業カテゴリは存在しないと述べる。**2 つの文書が同じ問い（条文の選択は誰が決めるか）に別の答えを書いている。**
**どちらに揃えるかはオーナーに尋ねてある（2026-09-24）。答えが出るまで、どのセッションも片方へ寄せない**
（この session が PLAN を書き換えようとした操作は、決定権の境界を AI の側へ寄せる編集として止められた）。
...
- **是正済み（2026-09-24）**: steward が「揃えてください」と答えたので、`image:license` を ACD-1.0 の全文 URL（JSON-LD の `license` と同じ値）へ変えた。**過去形の記録としてこの節は残す**（`against.md` #222）。
