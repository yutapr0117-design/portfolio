...
**基準 4 は「英語話者にとって文法的・統語的に明晰であること」であり、我々の唯一の生きたリスクである。**
明晰さそのものは機械で判定できないが、**その最大の敵である長文の割合は測れる**。
**instrument**（2026-09-11）: 80 桁の区切り線以降を本文とし、`. ; :` の後に大文字・引用符・括弧が
続く位置で文に割り、3 語未満を捨て、空白区切りトークンを数える。**承認済みライセンスの本文は
SPDX の `licenseText` から取得した。**

| ライセンス | 文数 | 語数 | 平均 | 中央値 | 最長 | **45 語超の割合** |
|---|---|---|---|---|---|---|
| **ACD-1.0** | 114 | 4,896 | 42.9 | 34 | 223 | **34.2%** |
| **ACD-1.1 草案** | 113 | 4,866 | 43.1 | 33 | 179 | **33.6%** |
| Apache-2.0 | 43 | 1,578 | 36.7 | 30 | 180 | 25.6% |
| MPL-2.0 | 80 | 2,435 | 30.4 | 26 | 108 | 18.8% |
| EPL-2.0 | 47 | 2,130 | **45.3** | 26 | **328** | 29.8% |
| OSL-3.0 | 46 | 1,636 | 35.6 | 33 | 119 | 23.9% |
| CDDL-1.0 | 81 | 2,523 | 31.1 | 25 | 177 | 17.3% |

**読み方（有利な側も不利な側も同じ精度で書く）**:

- **平均文長は範囲の中にある** —— EPL-2.0（45.3）より短く、Apache-2.0（36.7）より長い。
- **最長文も外れ値ではない** —— 223 語は EPL-2.0 の 328 語より短い。**1.1 草案では 179 語へ下がった。**
- **🔴 45 語超の割合は、比較した多節ライセンスの中で最も高い**（34% 対 17〜30%）。
  **ここだけは我々が最悪である。**

**したがって 1.1 の作業項目が 1 つ確定する** ——**長文の割合を下げる。**
**これは内容を削らずにできる**（1 文を 2 文に割る）が、**語数はわずかに増える** ——
**明晰さと短さは、ここで正面から競合する。** 選ぶのは明晰さである。理由は 2 つ:
**(a)** 基準 4 は承認の条件だが、長さは条件ではなく**審査者の注意力の問題**である（B3）。
**(b)** **B3 は既に測ってあり、削っても engagement の閾値（3,888 語）には届かない** ——
**届かない目標のために、届く目標を犠牲にしない。**

**この測定が establish しないもの**: **短い文は明晰であることを意味しない。**
文長は明晰さの代理指標であって明晰さそのものではなく、**非母語話者の起草に固有の誤り
（前置詞・冠詞・時制の一致）はこの測定に一切現れない。** それは依然として未検証である。
