...
There are **318** worked entries across this directory (measured 2026-09-09; the list is append-only, so treat it as a floor). This page exists so that you do not have to
...
| **Are you revising the text that is under discussion?** | **No. ACD-1.0 is frozen and pinned by SHA-256; a CI check fails the build if it moves.** A successor exists as a separate, frozen file — `ACD-1.1.txt`, whose first line says **FROZEN TEXT. NOT SUBMITTED FOR APPROVAL. NOT APPLIED TO THIS REPOSITORY.** — and states in its own header how many recorded defects it closed when frozen. **Its clause numbers differ from 1.0 in §1's definitions and in §15–§16** (mapped by defined term in `REVIEWERS.md`), so cite 1.0 from the 1.0 text. What changed, and what each change cost, is in `ACD-1.1-CHANGELIST.md`; the clause-by-clause mapping is in its §0.5 |
| **Are you posting to the lists right now?** | **Read `FROZEN.md`'s `POSTING-STATUS` marker — it is the single source, and this page deliberately does not copy it** (pages that copied it kept saying "paused" after posting resumed: `against.md` #195 / #221). The dated history: posting stopped on 2026-09-09 after the OSI Moderators announced that posts an AI wrote most of will be rejected (#119 / register B14; the verbatim notices are in `rounds/`), and resumed on 2026-09-17. **The one message the owner sent to `license-discuss` after resuming was rejected by moderation** (#215) |
