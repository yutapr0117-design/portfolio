...
- **Latest increments (2026-06-10) = `Stage 5 + 5-b page extraction + doc sync + orphan-comment cleanup + repository-wide audit`.** Sequence: (a) PR #13 acquired the Playwright visual-regression baseline (`e2e/portfolio.spec.js-snapshots/homepage-baseline-chromium-linux.png` / Playwright 1.60.0 / `chromium-linux`); (b) PR #14 fixed an img-src CSP violation surfaced by Playwright in the localhost env (preload absolute → relative URL); (c) PR #16 extracted `Router` + `PAGE_META` to `js/router.js` / `js/page-meta.js` (Stage 5, −193 lines); (d) PR #18 extracted `HiringRiskPage` / `RoleSplitPage` / `NotFoundPage` + 4 helpers to `js/pages.js` (Stage 5-b, −613 lines); (e) PR #19 synced architecture docs (file-size-budget, runbook §9, extraction-map §3.8/§3.9, maintainability-map changelog); (f) PR #20 collapsed orphan extraction-marker comments in `main.js` (−4 lines); (g) PR #21 closed a Check 47 verification-coverage gap (`js/pages.js` was missing from `_modules47` so its import/export bijection went un-enforced — silently green CI) and extended the export parser to recognise the named-list form `export { name1, name2 }`, then bulk-synced CLAUDE.md / eslint.config.mjs / major-update-readiness.md / check-repository-consistency-map.md drift. Cumulative `main.js`: 7,785 → 5,288 (**−32%**). All under closure-deps = none and AIDK Kernel byte-identical. (Exact check count / line counts / versions: read them with tools.)
- **Stage 5 baseline gate = unlocked.** The "blocking" condition from the prior handoff (baseline PNG missing) was resolved on 2026-06-10. Further physical splits of `main.js` (service rails: Storage/Store/State; remaining renderer / drawer / focus-trap / DiagnosticsRail / Theme・Brand; Meta Management) are now technically permissible — but each still demands `closure-deps = none` proof and schema-backward-compat (for Storage/Store/State). AIDK Isolated Kernel and View Transition Proxy remain **un-extractable** (Check 43 enforces structure).
