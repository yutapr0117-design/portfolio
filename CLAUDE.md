...
- **Stage 5 物理分割 = 完遂 (2026-06-10〜12)。`main.js` は 7,785 → **1,495 行 (−81%)** に縮小。** 13 個の小さな増分 (Stage 5-c〜5-o + 5-l) を factory pattern を確立しながら段階的に実施し、21 個の葉モジュール (`js/aidk-rails.js` / `apps.js` / `brand.js` / `components.js` / `constants.js` / `identity.js` / `meta-management.js` / `page-meta.js` / `pages.js` / `pure-utils.js` / `quiz-renderer.js` / `quiz/*` × 4 / `router.js` / `state.js` / `storage.js` / `store.js` / `theme.js` / `ui-components.js`) に分割した。詳細な増分一覧と PR # は `docs/architecture/repository-maintainability-map.md` の "Stage 5-c〜5-o + 5-l：service rails / UI / Apps / Quiz / AIDK Rails 完全抽出" セクション、ファクトリパターン確立の経緯は `docs/architecture/main-js-extraction-map.md` §3.10 を参照。
...
- **AIDK Isolated Kernel と View Transition Proxy は最終的に un-extractable で main.js に残置。** Check 43 が構造健全性を BLOCKING で機械強制している不可侵領域。残る `main.js` 1,495 行は AIDK Kernel + kernel 隣接の view-transition / render core / mobile drawer / focus trap + 入口層（init / event handlers / DOM clobbering interceptor）+ SITE_CONFIG + 各 factory の合成呼び出しで構成される。これらの追加抽出は kernel 構造との緊張関係を持つため別 phase で慎重に評価する。
