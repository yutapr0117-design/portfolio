...
def run(ctx):
    ...
    # **436 は行単位、436b は行内 60 字の窓 —— 意図的に違う（2026-09-13 に測って決めた）。**
    # 436b を窓にしたのは §7 の bullet (1 行 14,000 字超) で**無関係な「依頼」が違反を打ち消す**から。
    # **同じ理屈で 436 も窓にすべきか**を実測したところ、**新たに 14 件が上がり、14 件すべてが正当**
    # だった —— 内訳は (a) **本 Check 自身を説明する文**（map の行 / runbook の行 / §7 の解説）で、
    # **禁止語を定義のために列挙している**、(b) 行頭に `【SUPERSEDED・当時の記述】` を置いた
    # 歴史ブロック。**したがって 436 を窓にすると誤検出しか増えない。**
    # **差は例外マーカーの置かれ方にある**: 436 の例外語 (SUPERSEDED / 解決済み / 撤回) は
    # **行頭に置かれるブロック標識**で、436b の例外語 (依頼) は**文中に偶然現れる普通の語**である。
    # **既知の残存弱点（実測で今は空）**: 巨大な行が無関係な箇所に例外語を持ち、同じ行の別の場所に
    # 本物の違反がある場合、436 はそれを見逃す。**現在の内容では 0 件**なので窓化しない
    # （**見落としではなく測ったうえでの判断である**）。
    ...
    if _proj441.exists():
        ...
        _pats441 = [
            ...
            # **置換テキスト**。§16.3 の「固有ではない」は固有名詞の不在だけでなく
            # 「採用に本文編集が要らない」ことでもあり、提出パケット §4b はその 2 つを
            # 「0 and 0」として述べている。**固有名詞の側だけ CI が守り、置換テキストの側は
    ...
    _hits450 = []
    # **`LICENSES/rounds/` は対象外**: そこに在るのは受領した文面と外部の一次資料を
    # **無改変で**保存したものである。他人が書いた文が別スクリプトを含むのは正当で
    # (2026-09-13 に OSI 理事会議事録を保存したとき、2026-06-29 に否決された
    # **キリル文字表記の露語ライセンス名**がそのまま入った ——**この注記に実物を書き写さない。**
    # すぐ上の行が「文字を直接書くと本 file 自身がマッチする」と警告しており、
    # **最初に書いたときは、その警告の 20 行下でそれをやった**)、**この Check は「置換せよ」と要求するが、
    # このディレクトリでは byte を変えることこそが規約違反である。**
    # **除外して終わりにしない** —— 本 Check が守るのは「我々が書いた日本語に字形近似文字が
    # 紛れる」ことで、我々が書いていないテキストにはその危険が無い。rounds/ の header は
    # 我々が書くが、そこも含めて「原文がそう書いてある」以外の状態を作らないのが規約である。
    for _rel450 in _tracked450:
        if _rel450.startswith("LICENSES/rounds/"):
            continue
        _p450 = ROOT / _rel450
        try:
