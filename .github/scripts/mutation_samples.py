...
肥大化解消 (自走効率 + 保守性): runner ロジックと ~1,450 行の curated mutation データを分け、
増分ごとに追記するのは本ファイル (データ) だけにする。

- MUTATIONS     : consistency Check 安全網を検証する curated mutation 群 (過去修正した実 bug class)。
- E2E_MUTATIONS : behavior e2e 安全網を検証する curated mutation 群 (--e2e モード)。
...
if sys.version_info < (3, 10):
    # mutation_probe.py 等と同じく 3.10+ 前提 (import 側と揃える)。明示エラーで早期停止。
