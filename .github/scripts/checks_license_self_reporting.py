...
Check inventory (Check 45 enforces sync with the `# ── N.` sections in run()):
  ...
  475. **反証表の全行が、日付つきの状態を宣言していること (BLOCKING)**: `LICENSES/AS-OF.md` の
       反証条件の表は「観測が来たら、まずここを見る」「反証条件が満たされたら書き換えるのではなく
       書き足す」という規則を**自分で述べている**。**その規則を見る層が無かった** ——#192 は、
       行の反証条件が満たされてから **9 日間**誰も表を見に来なかったことを記録している。さらに
       2026-09-25 の掃引で **2 例目**が出た（#211 は 1 日前から register に在ったのに、
       gap 3 の行に届いていなかった）。
       **強制できるのは「状態が宣言されていること」までである** —— 条件が満たされたかどうかは
       世界についての判断で、機械には見えない。**だから射程を正直に書く**: 本 Check が防ぐのは
       「行が無言のまま古くなる」ことだけで、「気づかない」ことは防げない。
       各行に **🔴 満たされた / 🟡 未充足（近接あり）/ ⚪ 未充足 / ⚫ 前提未成立** のいずれかと
       **ISO 日付**を要求する。
       **⚠ 鮮度の閾値は置かない** —— 日数で赤くすると CI が勝手に腐り、赤の意味が失われる
       （`check_deployed_freshness.py` が版数を比較対象に選んだのと同じ理由）。
"""
