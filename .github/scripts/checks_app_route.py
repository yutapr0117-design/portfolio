...
Check inventory (Check 45 enforces sync with the `# ── N.` sections in run()):
  ...
  139. AppsPage app index ↔ router app whitelist bijection: AppsPage (js/components.js) is the canonical
       ...
       sidebar/Check 138). This Check enforces the AppsPage `apps` array and the router whitelist in BOTH
       directions: (a) missing (router - AppsPage) — a new app added to router/main.js/cmdk/sidebar but
       forgotten here becomes undiscoverable from the canonical index though it routes everywhere else;
       (b) extra (AppsPage - router) — a card added here for an app the router does not whitelist makes
       "開く" navigate to apps/<id> → not-found = a dead 404 card (the old one-directional Check passed
       this direction silently). It scopes to the AppsPage `const apps = [...]` array, parses its
       `id: '<app>'` entries, and asserts set-equality with the router apps, completing the app-route
       coherence mesh so all three producer surfaces (palette/sidebar/AppsPage) track the router whitelist. (BLOCKING)
...
def run(ctx):
    ...
    # ── 139. AppsPage app index ↔ router app whitelist bijection (BLOCKING) ─────────
    ...
    # 3 つ目の app-route producer 面。AppsPage の `apps` 配列と router whitelist の **双方向 (bijection)** を強制する:
    #   (a) 欠落 (router - AppsPage): router/main.js/cmdk/sidebar に新 app を足したが AppsPage を忘れると、
    #       その app は route できるのに canonical index から発見不能になる。
    #   (b) 余剰 (AppsPage - router): AppsPage に router 未登録の app card を足すと、「開く」が apps/<id> へ
    #       navigate → router whitelist 不一致で not-found 解決 = 開くと 404 の dead card になる (旧 Check は
    #       (a) 片側のみ強制で本方向は素通しだった・phantom card mutation で実証)。
    # AppsPage の `const apps = [...]` 配列に scope して `id: '<app>'` を parse し、router app 集合と一致
    # (bijection) することを強制する。これで 3 producer 面 (palette/sidebar/AppsPage) が全て router whitelist に
    # 追従し app-route coherence mesh が閉じる。
    ...
    if _router139.exists() and _comp139.exists():
        ...
        _missing139 = _router_apps139 - _appspage_apps139   # (a) router app 不在 = hub から発見不能
        _extra139 = _appspage_apps139 - _router_apps139      # (b) phantom app = 「開く」が not-found へ飛ぶ dead card
        check(
            bool(_router_apps139) and bool(_appspage_apps139) and not _missing139 and not _extra139,
            f"Check 139: AppsPage index が router の app 集合と bijection ({sorted(_router_apps139)})",
            f"Check 139: AppsPage index が router app whitelist と drift — "
            f"欠落(hub から発見不能): {sorted(_missing139)} / 余剰(「開く」が not-found へ飛ぶ dead card): {sorted(_extra139)}。"
            f"js/components.js の AppsPage `const apps = [...]` を router whitelist と一致させよ "
            f"(欠落は `{{ id: '<app>', title: ..., desc: ..., icon: ... }}` を追加 / 余剰は router 未登録 card を除去)"
