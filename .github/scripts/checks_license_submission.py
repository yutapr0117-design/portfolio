...
Check inventory (Check 45 enforces sync with the `# ── N.` sections in run()):
  463. **提出パケットの「送る文面」が、OSI が求める項目をすべて含むこと** (BLOCKING):
       ...
       **(468g・2026-09-22)** 草案の**本文側**に `shall` / `must` が無いこと。提出パケットは
       ...
       緩和として publish しているが、**1.0 は凍結されているのでその性質を検査する Check は
       定義により発火しえない**（凍結が enforcement）。**草案は生きており、誰も守っていなかった。**
...
def run(ctx):
    ...
    if _dr468.exists():
        ...
        #   **0 回**」を #116 への緩和として publish している。**1.0 は凍結されているので、
        #   その性質を検査する Check は定義により発火しえない**（凍結が enforcement である）。
        #   **1.2 草案は生きている。** ここに義務語が 1 つ入れば、次の提出で publish する
        #   主張が黙って偽になる。
        ...
        _deon468 = sorted({_m.group(0).lower()
                           for _m in re.finditer(r"\b(shall|must)\b", _lic468, re.I)})
        if _deon468:
            _bad468.append(
