...
def _wellknown_paths():
    ...
    """
    try:
        out = subprocess.run(["git", "ls-files", "-z", ".well-known"], cwd=ROOT,
...
def _check_dossier_reachable(base):
    ...
    """
    try:
        out = [p for p in subprocess.run(["git", "ls-files", "-z", "LICENSES"], cwd=ROOT,
    ...
    for rel in out:
        try:
            # 非 ASCII のパス (rounds/ の日本語名 3 件) は percent-encode しないと urllib が
            # UnicodeEncodeError を投げる。`-z` で正しい名前を得た後の、もう一段の落とし穴。
            _fetch_bytes(base + quote(rel))
        except Exception as e:  # noqa: BLE001
            # HTTP ステータスを残す: 2026-09-21 の赤では型名 (HTTPError) しか出ず、
            # 404 (配信されていない) と 429/5xx (一過性) を区別できなかった。
            bad.append(f"{rel} ({type(e).__name__} {getattr(e, 'code', '')})".replace(" )", ")"))
