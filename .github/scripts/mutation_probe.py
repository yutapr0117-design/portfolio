...
def _parse_shard() -> tuple[int, int]:
    ...
    分割は `index % n == i-1` の決定的な剰余で行う。

    **注意 (2026-08-17 の実測で訂正)**: 当初「剰余なら負荷が均される」と書いたが、
    4 分割の実測は 14m56s / 17m22s / **30m15s** / 15m46s で **shard 3 だけ約 2 倍**だった
    (30 分の timeout に到達して cancelled)。mutation ごとの所要は対象テストの重さで
    大きく変わり (長い待ちを含むテストは 1 mutation で 30 秒超)、剰余は**それを均さない**。
    均衡は分割数を増やして 1 shard あたりの粒度を細かくすることで確保する
    (8 分割で最悪ケースは半減する)。実測で偏りが再発したら分割数を上げること。
    """
    for a in sys.argv:
