/**
 * pages.js — ページレンダーコンポーネント（deps-none 候補）
 *
 * @fileoverview v80+ Stage 5: main.js から抽出したページ関数群。
 * HiringRiskPage / RoleSplitPage / NotFoundPage と、それらが使う補助関数
 * （impactRow / kpiRow / decisionFlow / riskCard）を含む。
 *
 * export:
 *   HiringRiskPage  — 採用リスク低減構造のページ
 *   RoleSplitPage   — Human vs AI 分担表
 *   NotFoundPage    — 404 ページ
 *
 * 不変条件:
 *   - 本モジュールは葉（ローカル ESM import ゼロ）。h / createIcon / Router を参照。
 *   - 補助関数（impactRow 等）は内部実装 — export しない。
 *   - State/Store/RouteState 依存なし（closure-deps = none）。
 */

// ===== Page Helper Functions (補助コンポーネント) =====
...
// ===== Export Public Pages =====
export { HiringRiskPage, RoleSplitPage, NotFoundPage };
