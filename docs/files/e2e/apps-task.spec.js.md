...
# e2e/apps-task.spec.js

## What

Task / Todo アプリの e2e。Task 追加と reload 永続・IME composition 中の Enter 無視・kanban 列移動と status 永続・削除・priority フィルタ・localStorage quota 超過時の graceful degradation・cross-tab sync (正常 / foreign-schema 無視)・snapshot restore の正規化・Todo の a11y 属性 leak なし・Todo add/complete/clear・Todo IME guard・clear-completed 無効化・filter 切替を検証する。

## Why

Task/Todo CRUD の永続性・IME 安全性・外部 ingestion 正規化 (#93/#295 class) を機能ゲートで守る。
