...
main.js
  ...
  └─ const State = createState({ CONSTANTS, Store, Storage, Toast })
       ...
       └─ State.updateSilently(fn) → ドラフト commit + scheduleSave するが notify せず
            (= 再描画しない)。高頻度 live-input (quiz 検索 / notes 入力) が毎キーストローク
            の全再描画で focused input を破棄し focus を失うのを防ぐ。呼び出し側が sub-DOM を
            手動更新する契約 (cf. ProjectsPage renderGrid)。
...
- **Check 52**: 210 行 ≤ 320
- **Check 130**: oninput ハンドラは `State.update(` を呼ばず `updateSilently(` を使う (focus-loss 防止)
