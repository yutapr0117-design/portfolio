...
Quiz Renderer factory module。`createQuizRenderer({deps, langOfText, CONSTANTS, loadQuizData })` を export。QuizPage + 4 domain (architecture / aws / pm / quality) の lookup table を含む。
...
main.js
  ...
  └─ ... + 3 domains
  └─ const QuizPage = createQuizRenderer({ h, createIcon, Toast, Router, State, loadQuizData, langOfText, CONSTANTS })
```
