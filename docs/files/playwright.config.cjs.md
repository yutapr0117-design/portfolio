...
> 残りはすべて肯定アサーションが先行しており健全。
>
> **⚠ 2026-08-24 にこの監査基準が弱すぎたと判明した。** 「先行する肯定アサーションがある」は
> 十分条件ではない —— **そのアサーションが前ルートの DOM で満たされないこと**まで要る。
> 実例: `All sidebar nav links resolve to valid (non-not-found) routes` は各 href へ `goto` した
> あと `page.locator('h1').first()` の可視を「描画確定」の代わりにしていたが、hash 遷移では
> **前ルートの h1 がそのまま残っている**ので即座に成立し、直後の `toHaveCount(0)` は *前のページ*
> に対して評価されていた。router から 1 ルートを落として NotFound へ落とす実測では、遷移直後の
> NotFound 見出しは **0 件（テストは PASS）**・settle 後は **1 件**。つまりこの BLOCKING gate は
> 自分が名乗っている「壊れた nav リンク」を**一度も検出できなかった**。
> **正しい基準**: 先行アサーションが *そのルートでしか成立しない* こと（見出しを `hasText` で
> 指す / 期待ルート名を `body[data-ai-state]` で待つ / 前ルートと**変わった**ことを待つ）。
> 41 箇所を再走査した結果、影響を受けていたのは上記 1 件のみで残りはルート固有だった。
>
> **2026-08-24 の続き: 同じ class を axe のルートループで踏んでいた（より重い）。**
> `a11y axe: ダークテーマの全ルートに render-neutral critical 違反が無い` は 1 つの test 内で
> 全 17 ルートを hash 遷移しながら走査するが、待ちが `#content h1, #content h2` の可視という
> **汎用**だったため、axe は**ちょうど 1 つ前のルート**を走査していた。実測（axe の結果に含まれる
> ノード量で判定・ページ側から読むと analyze 後の状態を見てしまうので誤る）: `#/quiz`
> （最大のコンテンツページ・64,395 bytes）は**一度も走査されておらず**、resume の内容を「quiz」
> として検査していた。**最後のルートも走査されない。**
> 決定的な証明: quiz の章見出しを h2→h4 にして `heading-order` 違反を注入すると、
> **旧待ちでは PASS・新待ちでは `#/quiz: heading-order(1)` で FAIL**。
> **なぜ contrast ループは無事だったか**: そちらは `reducedMotion: 'reduce'` を設定しており、
> View Transition が走らないぶん描画が axe の注入より先に終わる。**同じ書き方でも、
> アニメーションの有無で当たり外れが変わる**＝タイミングへの依存であって設計ではない。
> 対処は `gotoRouteSettled()`（`body[data-ai-state]` の `route` が前と変わり `loading` が false に
> なるのを待つ）。ルート名のハードコード表を持たずに済むのも利点 —— 表は必ず drift する。
