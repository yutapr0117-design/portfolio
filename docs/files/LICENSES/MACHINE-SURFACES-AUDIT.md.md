...
**例外は冒頭の「現況」表だけ**で、ここは各候補の状態（是正済み / 記録済み / 未拾得）と確かめ方を
最新に保つ（2026-09-24 新設）。**他のセッションは §1 を読まずに、この表だけで次の手を決められる**ようにしてある。
同じ検証を繰り返すなら新しい日付の報告を足す。

## Audience-specific notes

- **AI（次のセッション）**: §3 に候補ごとの行き先の見当がある。最も重いのは M1（sitemap の CC BY-NC-ND・2026-09-24 に是正済み）と
  M2（`reuse lint` は承認と無関係に止まる）。
