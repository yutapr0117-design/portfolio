...
`gap-measurements.md` §1.97 は同じ 141 本で「条件の有無」を数えたが、**gap そのものを 1 本ずつ当てた表は無かった**。
...
- 本文・表は **`.github/scripts/generate_gap_census.py` の生成物**（取得 `--fetch` / 生成 / 一致確認 `--check`）。
  **手で編集しない**。手で判定した分類（特許許諾の種類と範囲）はスクリプト内のデータで、
  `patent` を含むのに未分類の本が現れると生成が止まる（読まずに既定値へ落とさない）
