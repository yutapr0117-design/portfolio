...
### ドシエの到達性を足した理由（2026-09-09・6 層目）

**審査者が実際に辿るのは `LICENSES/README.md` の索引**であり、Check 459 が「`LICENSES/*.md` が
すべてそこから到達できる」ことを BLOCKING で強制している。**ただし Check 459 が見ているのは
リポジトリ内の相対リンクだけ**で、審査者が読むのは公開された URL である。

**ドシエは既存のどの集合にも入っていなかった** —— 凍結パスは pin された 3 file しか見ず、
資産パスは `index.html` の参照 ∪ `.well-known/` ∪ sitemap の `<loc>` から導出する。
**索引が指す先が公開面で 404 でも、リポジトリ側の Check は全部緑のまま**である。

対象は `git ls-files LICENSES` から**導出**する。ハードコードすると、2026-09-09 に 3 file
増えたときのように**追加のたびに黙って射程から外れる**。非 vacuity は実測済み
（URL に存在しない接尾辞を足すと 44 件すべてが RED になる）。
