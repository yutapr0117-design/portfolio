...
| 3. shipped バイト一致 | `style.css` / `main.js` / `sw.js` が **sha256 でリポジトリ一致** | AIO テキストの digest 不一致 |
...
対象は「壊れると全ページに影響する」中核 3 件に絞る（全 `js/*.js` を舐めると週次ジョブが遅くなり、
中核が一致していれば部分デプロイはほぼ確実に検出できる）。
