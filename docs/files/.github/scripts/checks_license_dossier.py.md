...
- **461b**（ADVISORY）/ **461c** — `LICENSES/*.md` の frontmatter `last-updated` が 14 日以上
  stale でないこと / すべての file が `last-updated` を宣言していること。
- **463** — 提出パケットの「送る文面」が OSI の review-process が求める項目をすべて含むこと。
- **464** — `errata.md` の全 `E<n>` が次版の変更リストに現れること。
- **465** — `LICENSES/rounds/` の在庫申告が実 file と一致すること。
- **466** — 審査者への案内が `README.md` の先頭に在ること。
- **467** / **467b** — 発信の停止が単一ソースと一致すること / 凍結解除後に面が「凍結中」と
  言い続けないこと。
- **468**（+ **468d** / **468e**）— 次版の草案が自分を述べ内部的に健全であること。
- **471** — **ドシエ内の参照記法が解決すること**（`rounds/` パス / `B<n>` register 項目 /
  `Check <N>`）。2026-09-14 の敵対的検証で**参照記法を 1 つずつ壊して測った**ところ、
  **7 形のうち 5 形が誰にも見られていなかった**。**残る 2 形**（裸の `` `file.md` `` と
  ドシエ内部の `§N.M`）は意味の判断が要るので**意図的に強制しない** ——
  その穴は `LICENSES/AUDIT-LEDGER.md` §7 に書いてある。

**⚠ Check 460（自己申告件数）はこの module に無い。** 2026-09-09 に
`checks_license_self_reporting.py` へ切り出した。**この一覧は 2026-09-14 まで 460 を挙げ続けて
いた** ——**一覧は項目が動いたときに古くなり、動かした本人には見えない**（Check 469 が守るのは
self-reporting module 側の face 一覧であって、この mirror ではない）。
