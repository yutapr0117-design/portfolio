---
file: .github/scripts/mutation_samples_e2e_archive.py
...
# .github/scripts/mutation_samples_e2e_archive.py

## What

**behavior (e2e) mutation の rotated アーカイブ (part 1)**。`E2E_MUTATIONS_ARCHIVE` を 1 つだけ export し、`mutation_samples.py` が

```
E2E_MUTATIONS = E2E_MUTATIONS_ARCHIVE + _E2E_TAIL      # 順序 = 時系列 (古 → 新)
```
...
- 本ファイルを新設した際、`mutation_samples.py` 側は `from mutation_samples_e2e_archive import E2E_MUTATIONS_ARCHIVE` と `E2E_MUTATIONS = E2E_MUTATIONS_ARCHIVE + _E2E_TAIL` の 2 行だけが変更点。**リストの実体は移動のみで、内容は 1 文字も変えていない**（rotate は非破壊であることが前提）。
