...
Type             : Increment notes — operating-model を実測検証して canon 強化し、AI 単独の ideation→triage→self-drive を実証、AI 自己生成の A 群改善案 6 件を自走実装（案8 = mutation-sample 検証含む）した run の詳細記録
...
- 案10（本ファイル）。案8（mutation-sample 検証）は §7 に結果を記録。
...
## 7. 案8 — mutation-sample による suite assertion-strength 検証（2026-06-21）

**reflect-then-organize の結論**: mutation testing の固有価値は「意味的に緩い assertion の検出」（vacuous-gate Check 39/55/56/114 は構造的空洞性は捕えるが緩い assertion は捕えない）で genuine。だが stryker + e2e 全 mutation は CI 的に prohibitive に重く付属物サイトには disproportionate。よって**重い CI infra を入れず、critical ロジックへ意図的 mutation を入れて対応 e2e が red 化する＝suite が捕捉することをローカルで一回検証**する軽量形を採った（新 dep ゼロ・CI コストゼロ）。

**結果: 4/4 mutation を suite が捕捉**（各 mutation は適用→対象 e2e 実行→fail 確認→即 revert、commit せず）:

| # | 注入した mutation | 期待して fail した e2e | 結果 |
|---|---|---|---|
| M1 | store.js `mergeProjectsWithDefaults` の slug 一意化ループを no-op 化 | colliding slugs yields unique slugs | ✅ caught (1 failed) |
| M2 | router.js `route.name = 'projects'` を別名に破壊 | Route projects renders | ✅ caught |
| M3 | apps.js NotesPage の `**bold**` tokenizer 正規表現を破壊 | Markdown notes app live-previews | ✅ caught |
| M4 | meta-management.js soft-404 の `noindex,nofollow` を `index,follow` に反転 | soft-404 protection | ✅ caught |

**解釈**: suite は構造的空洞性が無いだけでなく、critical paths（データ正規化 / routing / 安全レンダリング / AIO soft-404）で**意味的に正しく assert している**ことを実証。弱い assertion は本サンプルでは検出されず（test-strengthening 不要）。今後 critical ロジックを足す際は、同様の one-off mutation-sample で当該テストの assertion 強度を確認するのが軽量で有効（重い stryker CI は付属物サイトには不要との判断）。

これで A 群 6 案（案3 command palette / 案4 dashboard / 案6 notes app / 案8 mutation-sample / 案9 perf-budget / 案10 本ファイル）の自走実装を完了。
