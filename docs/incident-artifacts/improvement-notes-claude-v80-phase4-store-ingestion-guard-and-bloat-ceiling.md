...
PR-Range      : #568〜#574 (store ingestion class を per-instance fix→構造防止まで完遂)
Period        : 2026-07-05
...
### 4. ingestion-crash class の完全閉鎖 + 構造防止（#572 / #573 / #574）

store.js を「router/store edge-case」vein として深掘りした結果、`normalizeAppsData` の history 以外にも同一 class の未閉じ枝が複数見つかり、順に潰して最後に **構造防止 Check** で class 全体を封じた:

- **#572 🔴 normalizeProject の array field**: `tech`/`tags`/`highlights`/`relatedProjectIds`/`links` を `(raw.X || [])` = truthy 判定のみで扱っていた。非配列 truthy（文字列等）だと `.filter` が TypeError → validateAndNormalize 例外 → 全 ingestion 経路 crash。default の `proj()` builder は `Array.isArray` ガード済だが、**untrusted import を処理する normalizer 側が漏れていた**（producer は安全・consumer 正規化に穴）。5 field を `(Array.isArray(raw.X) ? raw.X : [])` へ。
- **#573 🔴 normalizeAppsData の task.tags**: `data.tasks` 自体は Array.isArray 済だが個々の `task.tags` が truthy 判定のみ（`(t.tags || []).filter`）。#572 の姉妹箇所で、**store.js の全 ingestion array-op を走査して見つけた最後の `(X || []).filter` 未ガード箇所**。既存 #568 e2e の seed に非配列 tags の task を追加して systematize。
- **#574 Check 364（BLOCKING・構造防止）**: per-instance に 3 回潰したが、新しい `(prop || []).map` が silent に再混入しうる。store.js に unsafe `(X || []).<throwing array-method>`（filter/map/forEach/some/reduce...）idiom が無いことを BLOCKING 強制。**肥大化 Check 363 と同じ「解消したら再発も防ぐ」規律の ingestion-safety 版**。安全形は `(Array.isArray(X) ? X : [])`。
  - **honest carve-out**: 実装当初、regex が `str.match(/.../g) || []).map`（`String.match` は Array|null 契約ゆえ安全）を誤検出した。直前を `\w`（プロパティアクセス末尾）に限定し method-call 結果（`)` 終端）を除外。**Check が実バグでなく設計を捕捉した→false-positive を honest に carve-out した実例**。非 vacuous は `(raw.zzz || []).map` 注入→RED で実測。

これで store.js の ingestion-crash class は **per-instance 修正（#568/#572/#573）+ 構造防止（#574）** の両面で完全閉鎖された。#93/#295/#561 から続く「外部 ingestion は全経路正規化を通せ」class の store.js における終点。
