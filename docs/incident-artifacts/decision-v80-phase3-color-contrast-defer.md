...
>
> この decision record の結論「オーナーが裁可した時に着手する」は **2026-08-18 に canon 側で
> 否定された**。オーナー明言（AI2AI.md STEP 3 / CLAUDE.md §7）—— 「**オーナー裁可が要る項目なんか
> 一切無い**」「**そもそも論として私は何かを制限したこと 0 だよ**」。**C5 は「人間がコードを
> 書かない」の意**であって、配色という設計判断を AI が defer する根拠にはならない。
> **「裁可待ち」という作業カテゴリは存在しない。**
>
> 実際この項目は #1157 で canon を是正した直後、**#1158 で解決された**（2 ブランド × 2 テーマ ×
> 16 ルートで color-contrast 違反 **85/80/84/82 → すべて 0**）。用途別の前景トークンへ分離する
> 方法を採っており、本文が想定した「単一の色を選び直す」形ではなかった。
>
> **本文は「当時どう判断したか」の記録として残す。規則として読まないこと。**
