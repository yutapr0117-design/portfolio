...
**Playwright baseline 取得状況（本 increment 時点）:** **未取得。** `e2e/portfolio.spec.js-snapshots/` に baseline PNG は存在しない。取得経路は、GitHub Actions の "Update Playwright Baseline Snapshots"（`update-playwright-snapshots.yml`）を人間が dispatch → 生成された PR をレビュー → snapshot PNG が意図通りであることを確認 → merge する経路が**唯一の正規ルート**である。ローカル/サンドボックスでは Chromium バイナリの DL が egress allowlist で遮断され `npm run test:e2e` が起動しないため、baseline をローカル生成して commit する経路は取れない（これはテストの欠陥ではなく環境制約）。生成は Playwright pin（`@playwright/test` = 1.60.0）で行い、CI（`playwright-regression.yml` が `npm ci` で pin 版を復元して比較）の版と一致させる。**この版一致は Check 51 が BLOCKING で機械強制する**（active runbook §7.4 の baseline 生成手順が名指しする版数 ↔ pin の一致検証。runbook §9 の Check 総数 51・本 increment で追加）。merge 後は本サブセクションの「取得状況」を「取得済み（PR #… / コミット …）」へ更新する。

**Stage 5 ゲート（再掲・不変）:** baseline PNG が現物に commit されるまで、render / router / view-transition / AIDK Kernel 抽出（Stage 5）には着手しない。上表の中〜高層も baseline 後が安全。安全弁はそれまで維持する。baseline 取得準備（手順確認・検証）は完了しており、人間による dispatch→PR→merge を待つ状態である。
