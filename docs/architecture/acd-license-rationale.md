...
**「AI 学習」という語を話すライセンスは存在しない**。一方 EU DSM 指令 (EU) 2019/790 第 4 条
は、権利者が「適切な方法（オンライン上の著作物については機械可読な方法）で留保」した場合に
TDM 例外を排除する構造を採る。つまり **沈黙は解釈に委ねられ、実務上は「留保あり」と読まれ
うる**。

ACD-1.0 §6 は逆向きの宣言を置く：留保を行わない／行うことを明示的に拒否する／既にある留保は
可能な限り撤回する／学習結果のモデル・重み・出力に一切の負担を課さない。**「留保しない」こと
を作品側から機械可読に宣言するライセンスは、現時点で他に無い。**
