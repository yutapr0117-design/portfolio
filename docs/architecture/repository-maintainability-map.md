...
| 層 | ファイル | 役割 | 変更時の注意 |
|---|---|---|---|
| **AIO正本層** | `llms-full.txt`（ground truth）, `AI2AI.md`（canonical handoff）, `llms.txt` + 3 alias, `.well-known/aio-manifest.json` | AI crawler / LLM 向けの権威ある真実源と pipeline 引き継ぎ | **C6**: エンティティ/権威テキスト・JSON-LD の本文変更は、真実で捏造でないこと・全公開面で食い違わないこと・変更後に digest を再生成することの 3 つを満たすこと (承認ゲートではない) |
...
| **検証層** | `.github/scripts/check_repository_consistency.py`, `check_aio_digests.py`, `check_binary_aio_metadata.py`, `check_css_stylelint.py`, `aio_monitoring.py`, `update_aio_digests.py`, `e2e/portfolio.spec.js`, `playwright.config.cjs`, `.github/workflows/*` | 整合性・回帰・AIO digest・lint の自動検査 | 検査を緩める変更は要判断。新規 invariant は Check 番号を付けて追記。canary トークンを編集する場合は published 面（`llms*`）と monitor 面（`aio_monitoring.py` / `check_public_deployment_freshness.py`）を同一文字列に保つこと（Check 44）。チェックを追加・採番変更する場合は docstring インベントリと `# ── N.` セクション見出しの両方を同時更新すること（Check 45 が両者の一致を BLOCKING で強制）。`package.json` の lint スクリプト（`lint`/`lint:js`）の JS 対象は同一集合かつディスク上の root ∪ js/ の実体と一致させること（Check 46）。`main.js` が `js/` 配下のローカル ESM モジュールから import する名前は、各モジュールの export と過不足なく一致させ、各モジュールは葉（import ゼロ）に保つこと（Check 47）。`update-playwright-snapshots.yml` が PR 作成ステップ（baseline を PR でコミット）を持つ限り、`contents: write` と `pull-requests: write` の両権限を宣言すること（Check 48）。`index.html` の JSON-LD で Person.worksFor が組織を参照する場合、その参照先 @id（OrganizationRole 経由のネストを含む）と同一 @graph 内の Organization ノードの @id を一致させ、宙吊り参照を作らないこと（Check 49）。ESLint は flat config（`eslint.config.mjs`・9.x 既定で導入・現在 v10 ピン）で運用し、`eslint.config.mjs` を残し・`package.json` の `lint` から旧 eslintrc 系フラグ（`--no-eslintrc`/`--config .eslintrc.json`/`--env`）を排し・旧 `.eslintrc.json` を置かないこと（Check 50。EOL の 8.x/eslintrc への逆戻りと vacuous-gate 再発の防止）。`eslint` と `@eslint/js` の devDependencies は同一メジャーに保つこと（Check 54。メジャー不一致は ESLint v10 系の解決衝突を招く・BLOCKING）。`index.html` の全 `<link rel="modulepreload">` の href はリポジトリ実体ファイルへ解決させること（Check 53。削除済みファイルへの dangling preload は 404 を生む・BLOCKING）。active runbook（`total-check-runbook.md`）の Playwright baseline 生成手順が名指しする Playwright 版数は `package.json` の `@playwright/test` pin と一致させること（Check 51。CI の比較版と生成版がずれると内容同一でも偽の視覚差分が出るため・BLOCKING。decision 記録・extraction-map 等の歴史層は対象外）。主要ファイルの行数は `docs/architecture/file-size-budget.md` の機械可読 BUDGET-DATA ブロックに記録し、整数予算を持つファイルは現行行数を予算以内に保つこと（Check 52。**ADVISORY＝非ブロッキング**。main.js は strong-advisory・AIO 正本/archive は ceiling なし。予算は文書側で単一管理しコードにハードコードしない） |
...
### quiz-domain-split + bloat-governance increment（v80+ — 静的データのドメイン別細分化＋肥大化予算の機械強制／本コミットで適用）

本 increment は、`プロンプト.md` / `改善文書.md` §7 が次回作業として収束させていた Package A（major-update-readiness）・Package B（file-size-budget + Check 52）・Package C（quiz-data 4 分割）・Package D（check-consistency-map）の 4 つを、競合せず非破壊で適用したものである。前 increment（baseline-gate-doc-hardening）はこれらを未実施で残しており、本 increment が genuine な次段階にあたる。

| 種別 | 対象ファイル | 変更内容 | 理由 |
|---|---|---|---|
| 静的データ分割（Stage 3-b・Package C） | `js/quiz-data.js`（削除）→ `js/quiz/aws-quiz-data.js` / `pm-quiz-data.js` / `quality-quiz-data.js` / `architecture-quiz-data.js`（新設）／`main.js`（import 書換） | 1,406 行の単一データファイルを 4 ドメイン葉モジュール（819/271/275/137 行）へ byte-equivalent 分割。`main.js` は 4 つを直接 import（aggregator は Check 47c 違反のため不採用）。データは行 slice で 1 バイトも変えず、ESM deep-equal（aws 30,415 / pm 3,445 / quality 3,432 / architecture 4,375 文字、すべて一致）で二重検証してから旧ファイル削除 | `改善文書.md` §5.3。純データゆえ baseline 不要・副作用ゼロ。最大の肥大化ファイル（quiz-data）を保守可能な単位へ |
| Check 47 拡張（Package C） | `.github/scripts/check_repository_consistency.py` | `_modules47` を 2 モジュール（pure-utils + quiz-data）から 5 モジュール（pure-utils + quiz 4）へ拡張。ループ検査構造のため bijection（47a/47b）と葉性（47c）が 4 quiz モジュールへ自動適用 | 分割で生じた手書き import/export 契約を機械強制（discover→systematize） |
| lint 被覆同期（Package C） | `package.json` | `lint`/`lint:js` の `js/quiz-data.js` を 4 ファイルへ更新（アルファベット順）。Check 46b の root ∪ js/ 再帰 glob と一致 | 分割後も全 shipped JS が gate 被覆下にあることを維持 |
| 肥大化予算（Package B） | `docs/architecture/file-size-budget.md`（新設）／`.github/scripts/check_repository_consistency.py`（**Check 52** 新設・ADVISORY） | 主要ファイルの行数予算を機械可読 BUDGET-DATA ブロックで定義し、Check 52 がパースして現行行数と照合。main.js は strong-advisory、AIO 正本/archive は ceiling なし。**非ブロッキング**（超過は warning のみ）。否定テスト（main.js 予算を 6000 へ下げ advisory 発火・exit 0 維持）で検証 | `改善文書.md` §6。予算は文書側で単一管理しコードにハードコードしない（Check 44/45/47 と同じ「文書と現実の一致」哲学の行数ドメイン適用） |
| 検証スクリプト構造地図（Package D） | `docs/architecture/check-repository-consistency-map.md`（新設） | Check 1〜52 を 6 カテゴリ（バージョン整合／AIO 正本／配信・SEO／歴史 honesty／構造パース・CI／自己統治）へ分類し、helper 抽出候補を識別。**物理分割はまだ行わない** | `改善文書.md` §5.4。将来の `lib/` 分割の準備。いきなり分割せず map から |
| メジャーアップデート準備文書（Package A） | `docs/architecture/major-update-readiness.md`（新設） | baseline dispatch→PR→merge→version bump→Stage 5 解禁の人間手順を集約。AI が実行できない権限境界を明記 | `改善文書.md` §7 Package A。Pattern 1/2 の「owner-action 準備文書」 |
| Check 45 自己整合の同時更新（Package B） | `.github/scripts/check_repository_consistency.py` | docstring インベントリへ「52.」、コード本体へ「# ── 52.」セクションを追加。Check 45a/45b/45c が「1..52」で緑であることを確認 | チェック追加時の docstring↔セクション bijection を維持（Check 45 強制） |
| 文書整合（数値同期） | `total-check-runbook.md` / `main-js-extraction-map.md` / `repository-maintainability-map.md`（本ファイル） | runbook §0.1「51 個」→「52 個」・§9 実測（consistency `OK:` 108→**118**〔Check 47 が +9 行=2→5 モジュール、Check 52 が +1 行〕・`npm run check` 全体 110→**120**・Check 総数 51→**52**・Check 52 説明追記）・§3 Layer 2 表「108/合計 110」→「118/合計 120」・§9 日付 06-02→06-07。extraction-map に §3.6（Stage 3-b）追記・Subject 行 6,355→6,360・§5 の lint clean 記述を 4 モジュールへ是正。本ファイルに本 changelog 追加 | 文書を実装と実測に同期（メモリ#29 の数値同期義務）。append-only 履歴は書き換えない |

非破壊性の核：AIO 正本層（`llms*` / `AI2AI.md` / `.well-known/*` / digest / `sitemap.xml` / `robots.txt` の本文）と binary（WebP/MP3）は 1 バイトも変更しておらず、digest 再生成は不要。`style.css` / `index.html` も不変。`main.js` は import ブロックの構造化（quiz 4 分割）に伴い 6,355→6,360 行（+5・機能追加なし・コメントのみ）。`npm run verify` は exit 0（**52 checks**・うち Check 52 は ADVISORY・all invariants hold・AIO digest passed・binary passed・Stylelint PASS・ESLint **0 errors / 120 warnings**＝分割は byte-equivalent ゆえ warning 不変）。`npm ci` 再現性・脆弱性 0 を確認済み。ESLint 警告の削減は本 increment では行っていない（残余 120 件は §3.4 の通り保護領域内または baseline ゲート対象で、安全に削れる低リスク件はゼロ）。
...
### console-fix + eslint-v10 + research-application increment（v80+ — quiz 分割が残した 404 の根本修正＋Wicle CSP＋ESLint v10 移行＋リサーチ全適用規律の明文化／本コミットで適用）

三つの依頼を受けて適用した increment である。(a) 公開現物のコンソールエラー 2 件の根本・非破壊修正、(b) トータルチェックと発見改善の適用、(c) リサーチ結果の全適用（ESLint v10 移行を含む）。加えてメタ要求として「リサーチは全適用までがリサーチ」という規律を、メモリと、メモリ未参照に備えてリポジトリ（`research-application-policy.md`）の双方へ明文化した。

| 種別 | 対象ファイル | 変更内容 | 理由 |
|---|---|---|---|
| console 404 根本修正（依頼a） | `index.html` | 削除済み `js/quiz-data.js` を指す dangling な `<link rel="modulepreload">` を、Stage 3-b 分割後の 4 モジュール（pure-utils ＋ quiz 4）へ更新。コメントも刷新 | quiz 分割時に main.js の import と package.json は更新したが modulepreload の参照先更新を漏らし、削除済みファイルへの preload が `404 (Not Found)` を出していた（私の漏れ＝認定）。byte-identical 維持の判断が参照ファイルに対して誤りだった |
| console CSP 根本修正（依頼a） | `index.html` | CSP `connect-src` に `https://cdn.wicle.io` を追加 | KARTE Action（Wicle）のエッジスクリプトが実験 config を `cdn.wicle.io` から fetch するが connect-src に無く CSP 違反でブロックされていた。KARTE 公式 CSP ガイド（`*.karte.io` 案内）を踏まえ、オーナーの個別サブドメイン列挙方針に合わせ specific endpoint を追加 |
| ESLint v10 移行（依頼c 中核） | `package.json` / `package-lock.json` / `eslint.config.mjs` | `eslint` 9.39.4→**10.4.1**・`@eslint/js` 9.39.4→**10.0.1**（exact pin・lock atomic 更新・7 パッケージ削除で依存ツリー軽量化・脆弱性 0）。`engines.node` を v10 要件 `^20.19.0 \|\| ^22.13.0 \|\| >=24` へ更新。`eslint.config.mjs` のヘッダと v10 bump 記録・stale な `js/quiz-data.js` 参照を是正 | リサーチ実適用。ESLint 9.x は 2026-08-06 EOL、v10 は flat config のみ。現物は既に flat config のため設定変更不要。**bump 前後で lint 出力 0 errors / 120 warnings（no-var:64 / curly:46 / no-shadow:10、内訳）が完全一致＝非破壊を実測証明** |
| Check 53 新設（modulepreload 参照解決・BLOCKING） | `.github/scripts/check_repository_consistency.py` | `index.html` の全 `<link rel="modulepreload" href>` がリポジトリ実体ファイルへ解決することを検証（same-origin 相対のみ・絶対 URL はスコープ外）。docstring↔`# ── N.` を Check 45 準拠で同時追記 | 今回の 404（分割で削除したファイルへの dangling preload）クラスを systematize。否定テストで `./js/DELETED-MODULE.js` 注入時に ERROR 発火を確認 |
| Check 54 新設（eslint↔@eslint/js メジャー一致・BLOCKING） | `.github/scripts/check_repository_consistency.py` | `package.json` の `eslint` と `@eslint/js` が同一メジャーであることを検証（メジャーのみ比較） | ESLint v10 の既知 footgun（メジャー不一致で解決衝突）を機械強制。否定テストで @eslint/js を 9 系に下げると ERROR 発火を確認 |
| Check 50 説明更新 | `.github/scripts/check_repository_consistency.py` | docstring・コメント・check メッセージの「ESLint 9.x flat config」を「flat config（9.x 既定・現在 v10）」へ更新（check ロジックは version-agnostic ゆえ不変） | v10 移行に伴う stale 回避 |
| リサーチ適用規律の明文化（メタ要求） | `docs/architecture/research-application-policy.md`（新設）／`CLAUDE.md` | 「リサーチは全適用までがリサーチ」「『リサーチするか？』と問わない」「確認だけで止めない＝適用 / 理由付き保留 / 現行性検証の三分類で必ず反映」を明文化。`CLAUDE.md` §0 MANDATORY に必読ポインタを追加し §3 の stale 現在状態（main.js 行数・検査数・ESLint 版）を是正 | メモリは関連性依存で未参照の可能性があるため、リポジトリ側にも落として冗長なやり取りの再発を構造的に防ぐ（オーナー明示） |
| AIPREF 不適用の判断記録（依頼c） | （適用せず・`research-application-policy.md` §3C に記録） | IETF AIPREF `Content-Usage` は適用しない | 「draft だから」ではなく、robots.txt が学習ボットを意図的に許可する最大許可方針であり、restriction 機構の AIPREF とは戦略が逆のため（改善に繋がらない） |
| WCAG 2.2 / CWV 保留の判断記録（依頼c） | （適用せず・`research-application-policy.md` §3C に記録） | target size 24×24・focus appearance 等は保留 | `style.css`/render 変更を要し、Playwright baseline 未取得では非破壊性を証明不能（安全ゲート）。baseline 取得後に着手 |
| 文書整合（数値・版数同期） | `total-check-runbook.md` / `repository-maintainability-map.md`（本ファイル）/ `main-js-extraction-map.md` | runbook §0.1「52 個」→「54 個」・§9 実測（consistency `OK:` 118→**120**・全体 120→**122**・Check 総数 52→**54**・Check 53/54 説明追記・stale quiz-data.js→4 モジュール）・§3 Layer2「118/120」→「120/122」・日付。本ファイル検証層セルに Check 53/54、現行 lock eslint 9.39.4→10.4.1 を反映。extraction-map の ESLint/modulepreload 現状記述を是正 | 文書を実装と実測に同期（メモリ#28 の数値同期義務）。append-only 履歴は書き換えない |

非破壊性の核：AIO 正本層（`llms*` / `AI2AI.md` / `.well-known/*` / digest / `sitemap.xml` / `robots.txt` の本文）と binary（WebP/MP3）と `style.css` は 1 バイトも変更しておらず、digest 再生成は不要。`index.html` は console エラー 2 件の根本修正（modulepreload 5 化＋connect-src に cdn.wicle.io）のみ変更（ai:version/ai:last-modified は不変＝Check 1/2/3/17 緑・version bump せず v74 内 hygiene）。`main.js` は不変。`npm run verify` は exit 0（**54 checks**・うち Check 52 のみ ADVISORY・all invariants hold・AIO digest passed・binary passed・Stylelint PASS・ESLint **0 errors / 120 warnings** 不変）。`npm ci` 再現性・脆弱性 0 を確認済み。
