...
| `LICENSES/ACD-1.0.review-corpus.md` | 636 | 900 | `advisory` | **`license-review` / `license-discuss` のアーカイブ全体を取得して測ったこと**（承認の基準 8 条・委員会が述べた規則・決議の分布・提出から決定までの所要・手続きの実態）。**2026-09-13 に `review-precedents.md` が Check 365 の 1,000 行に当たったので切り出した** —— **圧縮ではなくテーマ分割**で、**節番号は動かしていない**（既存参照を保つため）。**新しい節の行き先の規則**: 個別スレッドの読み → `review-precedents.md` / アーカイブ全体の測定 → 本 file |
| `LICENSES/ACD-OSI-BOTTLENECKS.md` | 808 | 900 | `advisory` | **承認を阻害する要因の唯一の canonical 一覧**（B1〜B14 + 2026-09-11 の掘削結果）。**2026-09-11 に 800 行を越え Check 454 が「早期警告が存在しない」と検出したので登録した** —— **黙らせるためではなく、BLOCKING (1,000) の手前で鳴らすため**に advisory を 900 に置く。越えたら**圧縮ではなくテーマ分割**で対処する（分析の切り出し先は `ACD-1.0.review-precedents.md`）|
