...
| `LICENSES/ACD-1.0.comparison.md` | 749 | 900 | `advisory` | 既存ライセンスとの逐条比較、および**外部レビューの実例から還元した節**（§1.45 AI-MIT / §1.46 PBZC / §1.47 MIT-I / §1.48 Modified 0BSD）。**2026-09-07 に 800 行を越え Check 454 が発火して登録された。** advisory は BLOCKING (1000) の 100 行手前。**分割の disposition**: 「既存ライセンスとの比較」と「審査アーカイブからの還元」は読者も用途も違うので、次に越えたら §1.45〜§1.48 を `ACD-1.0.review-precedents.md` へ切り出す。今切らないのは、still 1 テーマ（「他の instrument に照らして ACD はどこに立つか」）として読めるうちは分割が索引と相互参照を増やすだけだから |
