...
| `.github/scripts/checks_license_dossier.py` | 638 | 600 | `advisory` | ACD-1.0 ドシエ (LICENSES/) の自己整合 Check 群（投稿先 458 / 到達性 459 / **入口の言語 459b** / 自己申告件数 460 / 日付の鮮度 461b）。**2026-09-05 に `checks_governance_sync.py` が 966 行で advisory を鳴らしたので切り出した** —— 圧縮で黙らせず、いま触っている塊を出す。ドシエ側の Check は OSI 審査の往復が続くかぎり育つので、800 の登録義務ラインより手前の 600 で早期警告する |
