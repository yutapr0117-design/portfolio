...
| `LICENSES/BLIND-SPOTS-LOG.md` | 344 | 900 | `advisory` | **`BLIND-SPOTS.md` の適用の日次ログ**（いつ、どの次元を使い、何が出たか）。**2026-09-23 に分割** ——本体が Check 365 の1,000 行に当たったとき、**圧縮ではなく伸び続ける側を切り出した**（日次ログは増分ごとに必ず伸びるが、方法論は伸びない）。**越えたらまた同じ基準で割る** |
