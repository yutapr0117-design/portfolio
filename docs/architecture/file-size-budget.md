...
| `LICENSES/ACD-1.0.comparison.md` | 869 | 900 | `advisory` | 既存ライセンスとの逐条比較、および**外部レビューの実例から還元した節**（§1.45 AI-MIT / §1.46 PBZC / §1.47 MIT-I / §1.48 Modified 0BSD）。**2026-09-07 に 800 行を越え Check 454 が発火して登録された。** advisory は BLOCKING (1000) の 100 行手前。**分割の disposition**: 「既存ライセンスとの比較」と「審査アーカイブからの還元」は読者も用途も違うので、次に越えたら §1.45〜§1.48 を `ACD-1.0.review-precedents.md` へ切り出す。今切らないのは、still 1 テーマ（「他の instrument に照らして ACD はどこに立つか」）として読めるうちは分割が索引と相互参照を増やすだけだから |
...
| `LICENSES/BLIND-SPOTS.md` | 727 | 900 | `advisory` | **自分の欠陥を探すのに使った次元と、まだ使っていない次元の記録**。**盲点は「まだ探していない場所」ではなく「まだ使っていない測り方」に在る**という観察が中心で、増分ごとに 1 節ずつ伸びる。**2026-09-20 に 800 行を超えて Check 454 が「早期警告が存在しない」と止めた** ——予算が無い file は advisory にかからず、**OK からいきなり 1,000 行の BLOCKING へ飛ぶ**。次に advisory へ当たったら、使い終わった次元の記録を主題で切り出す |
...
| `LICENSES/ACD-1.2-DRAFT.txt` | 801 | 900 | `advisory` | **次版の草案**（ヘッダの変更一覧 + 本文 80 条）。ヘッダは変更 1 件ごとに理由まで書くので伸びる。**本文の語数は register B3 の費用として別に数える**（Check 468c が申告と実測の一致を強制）。900 を越えたら、ヘッダの理由づけを `ACD-1.2-CHANGELIST.md` へ寄せて短くする |
