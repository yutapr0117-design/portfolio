...
| `LICENSES/ACD-1.0.board-decisions.md` | 152 | 700 | `advisory` | **OSI 理事会の決定**（議事録という一次資料からの分析）。**2026-09-13 に `review-corpus.md` が 908 行で advisory (900) を越えたので切り出した** —— 圧縮せず、いま触っている塊を出す方針。**分ける基準は「資料の種類」** ——リストのアーカイブ全体の測定は `review-corpus.md`、個別スレッドの読みは `review-precedents.md`、**理事会という別の発話主体の決定**はここ。**同じ出来事について、リストは議論を、理事会は決定を残す。** 公開議事録は毎月増えるので育つ側の file である |
| `LICENSES/ACD-OSI-BOTTLENECKS.md` | 833 | 900 | `advisory` | **承認を阻害する要因の唯一の canonical 一覧**（B1〜B14 + 2026-09-11 の掘削結果）。**2026-09-11 に 800 行を越え Check 454 が「早期警告が存在しない」と検出したので登録した** —— **黙らせるためではなく、BLOCKING (1,000) の手前で鳴らすため**に advisory を 900 に置く。越えたら**圧縮ではなくテーマ分割**で対処する（分析の切り出し先は `ACD-1.0.review-precedents.md`）|
