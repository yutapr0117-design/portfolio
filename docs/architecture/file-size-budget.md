...
| `e2e/apps-settings-io.spec.js` | 619 | 900 | `advisory` | behavior e2e spec。Check 365 の 1,000 行 BLOCKING 上限の手前で警告する早期警告層 |
| `e2e/apps-settings.spec.js` | 886 | 900 | `advisory` | behavior e2e spec。Check 365 の 1,000 行 BLOCKING 上限の手前で警告する早期警告層 |
...
>
> **本日の累積: 716,800 → 722,400（+5,600・+0.78%）。** 6 回すべて実バグ修正で、**5 回はデータの silent な喪失**に関わる（偽成功 / 非表示設定の脱落 / 貼り付けの無言切り捨て / スナップショット上書き / 全置換 / 復元点の無言削除）。
