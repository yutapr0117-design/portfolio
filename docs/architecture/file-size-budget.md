...
| `LICENSES/ACD-1.0.review-corpus.md` | 828 | 900 | `advisory` | **`license-review` / `license-discuss` のアーカイブ全体を取得して測ったこと**（承認の基準 8 条・委員会が述べた規則・決議の分布・提出から決定までの所要・手続きの実態）。**2026-09-13 に `review-precedents.md` が Check 365 の 1,000 行に当たったので切り出した** —— **圧縮ではなくテーマ分割**で、**節番号は動かしていない**（既存参照を保つため）。**新しい節の行き先の規則**: 個別スレッドの読み → `review-precedents.md` / アーカイブ全体の測定 → 本 file |
| `LICENSES/ACD-1.0.review-rules.md` | 395 | 700 | `advisory` | **OSI が公表しているページを原典で読んだ記録**（§1.70 承認基準 8 条 / §1.76 要件リスト / §1.81 CoC 棚卸し / §1.82 本文変更の手続き / §1.86 CoC 逐条）。2026-09-15 に corpus が 1,000 行の BLOCKING に当たったので、**圧縮せず資料の種類で切り出した** —— 公表ページは規則を、リストは議論を、理事会は決定を残す |
| `LICENSES/BLIND-SPOTS.md` | 805 | 900 | `advisory` | **自分の欠陥を探すのに使った次元と、まだ使っていない次元の記録**。**盲点は「まだ探していない場所」ではなく「まだ使っていない測り方」に在る**という観察が中心で、増分ごとに 1 節ずつ伸びる。**2026-09-20 に 800 行を超えて Check 454 が「早期警告が存在しない」と止めた** ——予算が無い file は advisory にかからず、**OK からいきなり 1,000 行の BLOCKING へ飛ぶ**。次に advisory へ当たったら、使い終わった次元の記録を主題で切り出す |
| `LICENSES/ACD-1.0.board-decisions.md` | 238 | 700 | `advisory` | **OSI 理事会の決定**（議事録という一次資料からの分析）。**2026-09-13 に `review-corpus.md` が 908 行で advisory (900) を越えたので切り出した** —— 圧縮せず、いま触っている塊を出す方針。**分ける基準は「資料の種類」** ——リストのアーカイブ全体の測定は `review-corpus.md`、個別スレッドの読みは `review-precedents.md`、**理事会という別の発話主体の決定**はここ。**同じ出来事について、リストは議論を、理事会は決定を残す。** 公開議事録は毎月増えるので育つ側の file である |
| `LICENSES/ACD-OSI-BOTTLENECKS.md` | 407 | 700 | `advisory` | **承認を阻害する要因の唯一の canonical 一覧**（B1〜B14 + 2026-09-11 の掘削結果）。**2026-09-11 に 800 行を越え Check 454 が「早期警告が存在しない」と検出したので登録した** —— **黙らせるためではなく、BLOCKING (1,000) の手前で鳴らすため**に advisory を 900 に置く。越えたら**圧縮ではなくテーマ分割**で対処する（分析の切り出し先は `ACD-1.0.review-precedents.md`） **2026-09-13 に advisory を 900 → 950 へ上げた。理由を測ってから上げている** —— 当日の追加は B4 / B5 / B11 への「解消したときに失うもの」欄 **計 54 行**で、**それを除くと 860 行＝予算内**。つまり**超過は冗長さではなく分析の分**である。**圧縮も検討して退けた**: 冒頭 30 行の掘削サマリは「register の各項目はこれを前提に読む」という読み順を与えており、20 行を買うために削るのは**圧縮で黙らせる**ことにあたる。**分割も退けた** —— 本文が「**この文書が唯一の canonical なボトルネック一覧である**」と宣言しており、**割ると単一ソース性そのものを壊す**。**次に鳴ったときの分割線は決めてある**: **索引 + status（短い面）** と **bottleneck ごとの深い分析（長い面）** で割る |
| `LICENSES/ACD-OSI-BOTTLENECKS-EXTERNAL.md` | 857 | 800 | `advisory` | **register のうち「外の答えに依存する 7 項目」の深い分析**（B1+B2 / B4 / B5 / B7 / B10 / B11 / B14）。**2026-09-14 に register が 957 行で advisory (950) を鳴らしたので分割した。** **⚠ 分割線は前日に測らずに書いた線とは違う** —— 「索引+status / 深い分析」で割ると **85 / 871** で受け皿が即座に再び鳴る。**測ってから「行為主体で割る」へ変えた（370 / 586）。** **索引・分類・集計は分離しない** ——本文が「唯一の canonical なボトルネック一覧」と宣言しており、一覧＝索引の表だから |
| `LICENSES/PEER-REVIEW-WATCH.md` | 847 | 900 | `advisory` | 同時代 instrument（OpenMDW / ModelGo）の経過観察と、**アーカイブ取得の手順・取得済みの窓・そこで得たもの**。2026-09-15 に両アーカイブの全期間取得を実測したので、**窓は取得の制約ではなく「何を測ったか」の記録**になった |
...
| `.github/scripts/checks_license_dossier.py` | 826 | 800 | `advisory` | ACD-1.0 ドシエ (LICENSES/) の自己整合 Check 群（投稿先 458 / 到達性 459 / **入口の言語 459b** / 自己申告件数 460 / 日付の鮮度 461b）。**2026-09-05 に `checks_governance_sync.py` が 966 行で advisory を鳴らしたので切り出した** —— 圧縮で黙らせず、いま触っている塊を出す。ドシエ側の Check は OSI 審査の往復が続くかぎり育つので、800 の登録義務ラインより手前で早期警告する。**2026-09-06 に 700 へラチェット** —— 超過分は Check 463（送る文面が OSI の要求 10 項目を含む）の追加そのもので、コメントを削っても収まらない ＝**冗長さではなく機構の分**。上げた以上は次に鳴ったとき「また上げる」でなく分割を検討する |
...
| `.github/scripts/checks_license_self_reporting.py` | 692 | 700 | `advisory` | Check 460（ドシエの自己申告件数）と Check 469（自己申告の**列挙**が実装と一致すること）を持つ split module。**面の数はここに書かない** —— 実測 2026-09-13 に本欄が「12 面」、module の docstring inventory が「(a)〜(k) の 11 面」、実装が **16 面**と、3 つの数が 3 つとも違っていた。**単一ソースは module の docstring inventory で、Check 469 (a) がそれを実装と双方向照合する。****2026-09-09 に `checks_license_dossier.py` が advisory (800) を越えたので切り出した** —— face (l) を足した増分の中で、圧縮せず「いま触っているクラスタ」を出す方針（`checks_mutation_integrity.py` を `checks_maintainability.py` から切り出したのと同じ手）。**分割の disposition**: 面は増える一方なので、次に advisory を越えたら**入口ページ面 (g)(h) と本文の数値面 (a)〜(f)** で割る —— 前者は「読者が最初に見るページ」、後者は「文書群の規模」で、drift の起き方が違う |
