...
| `LICENSES/ACD-1.0.comparison.md` | 847 | 900 | `advisory` | 既存ライセンスとの逐条比較、および**外部レビューの実例から還元した節**（§1.45 AI-MIT / §1.46 PBZC / §1.47 MIT-I / §1.48 Modified 0BSD）。**2026-09-07 に 800 行を越え Check 454 が発火して登録された。** advisory は BLOCKING (1000) の 100 行手前。**分割の disposition**: 「既存ライセンスとの比較」と「審査アーカイブからの還元」は読者も用途も違うので、次に越えたら §1.45〜§1.48 を `ACD-1.0.review-precedents.md` へ切り出す。今切らないのは、still 1 テーマ（「他の instrument に照らして ACD はどこに立つか」）として読めるうちは分割が索引と相互参照を増やすだけだから |
...
| `.github/scripts/checks_license_self_reporting.py` | 731 | 700 | `advisory` | Check 460（ドシエの自己申告件数）と Check 469（自己申告の**列挙**が実装と一致すること）を持つ split module。**面の数はここに書かない** —— 実測 2026-09-13 に本欄が「12 面」、module の docstring inventory が「(a)〜(k) の 11 面」、実装が **16 面**と、3 つの数が 3 つとも違っていた。**単一ソースは module の docstring inventory で、Check 469 (a) がそれを実装と双方向照合する。****2026-09-09 に `checks_license_dossier.py` が advisory (800) を越えたので切り出した** —— face (l) を足した増分の中で、圧縮せず「いま触っているクラスタ」を出す方針（`checks_mutation_integrity.py` を `checks_maintainability.py` から切り出したのと同じ手）。**分割の disposition**: 面は増える一方なので、次に advisory を越えたら**入口ページ面 (g)(h) と本文の数値面 (a)〜(f)** で割る —— 前者は「読者が最初に見るページ」、後者は「文書群の規模」で、drift の起き方が違う  **2026-09-22: 732 行で advisory (700) を越えた。予算は上げない** ——規律は「全部消しても収まらない＝超過は機能の分」を確認してから上げることで、**実測するとコメントを全部消せば 599 行で収まる**＝条件を満たさない。**だが圧縮もしない**: 各 face の WHY は「その face がなぜ在るか」の唯一の記録で、消せば次のセッションが同じ drift を再発見する。**次に鳴ったときの分割線を先に決めておく** ——**Check 460（数の申告）と Check 469（列挙の申告）で割る**（469 は約 90 行で独立している）。**先に決めるのは、改訂の圧力下で決めると都合よく割るから。**|
...
.github/scripts/checks_license_self_reporting.py | 700 | advisory
