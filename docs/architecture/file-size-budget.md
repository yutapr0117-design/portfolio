...
| `LICENSES/ACD-1.0.comparison.md` | 658 | 900 | `advisory` | 既存ライセンスとの逐条比較、および**外部レビューの実例から還元した節**（§1.45 AI-MIT / §1.46 PBZC / §1.47 MIT-I / §1.48 Modified 0BSD）。**2026-09-07 に 800 行を越え Check 454 が発火して登録された。** advisory は BLOCKING (1000) の 100 行手前。**分割の disposition**: 「既存ライセンスとの比較」と「審査アーカイブからの還元」は読者も用途も違うので、次に越えたら §1.45〜§1.48 を `ACD-1.0.review-precedents.md` へ切り出す。今切らないのは、still 1 テーマ（「他の instrument に照らして ACD はどこに立つか」）として読めるうちは分割が索引と相互参照を増やすだけだから |
| `LICENSES/ACD-1.0.review-precedents.md` | 557 | 900 | `advisory` | `license-review` / `license-discuss` の記録から読み取った、ACD-1.0 に当たる指摘と当たらない指摘（切り出し時点で §1.45〜§1.50、以後 append）。**2026-09-08 に `comparison.md` が advisory を越えたので切り出した** —— 読者も用途も違う（前者は「どれを選ぶべきか」、本書は「この提出は何を言われるか」）。**節番号は変えていない**ので既存の `§1.4x` 参照はそのまま解決する。advisory は BLOCKING (1000) の 100 行手前 |
...
| `.github/scripts/checks_license_self_reporting.py` | 646 | 700 | `advisory` | Check 460（ドシエの自己申告件数）と Check 469（自己申告の**列挙**が実装と一致すること）を持つ split module。**面の数はここに書かない** —— 実測 2026-09-13 に本欄が「12 面」、module の docstring inventory が「(a)〜(k) の 11 面」、実装が **16 面**と、3 つの数が 3 つとも違っていた。**単一ソースは module の docstring inventory で、Check 469 (a) がそれを実装と双方向照合する。****2026-09-09 に `checks_license_dossier.py` が advisory (800) を越えたので切り出した** —— face (l) を足した増分の中で、圧縮せず「いま触っているクラスタ」を出す方針（`checks_mutation_integrity.py` を `checks_maintainability.py` から切り出したのと同じ手）。**分割の disposition**: 面は増える一方なので、次に advisory を越えたら**入口ページ面 (g)(h) と本文の数値面 (a)〜(f)** で割る —— 前者は「読者が最初に見るページ」、後者は「文書群の規模」で、drift の起き方が違う |
