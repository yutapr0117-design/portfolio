...
| `LICENSES/BLIND-SPOTS.md` | 855 | 900 | `advisory` | **自分の欠陥を探すのに使った次元と、まだ使っていない次元の記録**。**盲点は「まだ探していない場所」ではなく「まだ使っていない測り方」に在る**という観察が中心で、増分ごとに 1 節ずつ伸びる。**2026-09-20 に 800 行を超えて Check 454 が「早期警告が存在しない」と止めた** ——予算が無い file は advisory にかからず、**OK からいきなり 1,000 行の BLOCKING へ飛ぶ**。次に advisory へ当たったら、使い終わった次元の記録を主題で切り出す |
