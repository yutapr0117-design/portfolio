...
| `.github/scripts/checks_governance_sync.py` | 691 | 950 | `advisory` | 統治文書の同期 Check 群（規範層の defer 理由 436 / venue の単一ソース 458 / ライセンス文書の到達性 459 / **自己申告件数** 460 ほか）。**2026-08-27 に Check 454（危険域 >800 行は登録必須）が本 file 自身へ発火して登録された** —— 同日に作った早期警告が、作った本人の module を最初に捕まえた実例 |
...
| `.github/scripts/checks_license_self_reporting.py` | 662 | 700 | `advisory` | Check 460（ドシエの自己申告件数）と Check 469（自己申告の**列挙**が実装と一致すること）を持つ split module。**面の数はここに書かない** —— 実測 2026-09-13 に本欄が「12 面」、module の docstring inventory が「(a)〜(k) の 11 面」、実装が **16 面**と、3 つの数が 3 つとも違っていた。**単一ソースは module の docstring inventory で、Check 469 (a) がそれを実装と双方向照合する。****2026-09-09 に `checks_license_dossier.py` が advisory (800) を越えたので切り出した** —— face (l) を足した増分の中で、圧縮せず「いま触っているクラスタ」を出す方針（`checks_mutation_integrity.py` を `checks_maintainability.py` から切り出したのと同じ手）。**分割の disposition**: 面は増える一方なので、次に advisory を越えたら**入口ページ面 (g)(h) と本文の数値面 (a)〜(f)** で割る —— 前者は「読者が最初に見るページ」、後者は「文書群の規模」で、drift の起き方が違う |
...
| `.github/scripts/checks_aio_config.py` | 870 | 950 | `advisory` | AIO 設定面の Check 群。同上の cohort |
