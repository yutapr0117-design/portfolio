...
| `LICENSES/BLIND-SPOTS.md` | 997 | 900 | `advisory` | **自分の欠陥を探すのに使った次元と、まだ使っていない次元の記録**。**盲点は「まだ探していない場所」ではなく「まだ使っていない測り方」に在る**という観察が中心で、増分ごとに 1 節ずつ伸びる。**2026-09-20 に 800 行を超えて Check 454 が「早期警告が存在しない」と止めた** ——予算が無い file は advisory にかからず、**OK からいきなり 1,000 行の BLOCKING へ飛ぶ**。次に advisory へ当たったら、使い終わった次元の記録を主題で切り出す |
...
| `LICENSES/ACD-OSI-BOTTLENECKS-EXTERNAL.md` | 897 | 800 | `advisory` | **register のうち「外の答えに依存する 7 項目」の深い分析**（B1+B2 / B4 / B5 / B7 / B10 / B11 / B14）。**2026-09-14 に register が 957 行で advisory (950) を鳴らしたので分割した。** **⚠ 分割線は前日に測らずに書いた線とは違う** —— 「索引+status / 深い分析」で割ると **85 / 871** で受け皿が即座に再び鳴る。**測ってから「行為主体で割る」へ変えた（370 / 586）。** **索引・分類・集計は分離しない** ——本文が「唯一の canonical なボトルネック一覧」と宣言しており、一覧＝索引の表だから |
| `LICENSES/PEER-REVIEW-WATCH.md` | 886 | 900 | `advisory` | 同時代 instrument（OpenMDW / ModelGo）の経過観察と、**アーカイブ取得の手順・取得済みの窓・そこで得たもの**。2026-09-15 に両アーカイブの全期間取得を実測したので、**窓は取得の制約ではなく「何を測ったか」の記録**になった |
...
| `.github/scripts/checks_license_submission.py` | 510 | 800 | `advisory` | **審査者が受け取る面だけを検査する Check 群**（463 送る文面 / 468 次版草案 / 472 条項引用 / 473 来歴の開示）。**2026-09-22 に `checks_license_dossier.py` が 914 行で advisory (800) を越えたので切り出した** ——**圧縮ではなくテーマ分割**。**分ける基準は「対象が誰の手に渡るか」**で、残した側の損害は「記録が古い」、こちら側は「**審査者が誤った物を読む**」。越えたらまた同じ基準で割る |
