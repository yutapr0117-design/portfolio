...
| `LICENSES/ACD-1.0.submission.md` | 928 | 950 | `advisory` | OSI / SPDX へ送る提出パケット本体。**2026-09-06 に 800 行を越え Check 454 が発火して登録された** —— 早期警告の無い file を作らないための登録義務ライン。advisory は BLOCKING (1000) の 100 行手前に置く。**分割の disposition**: この file は「そのまま送れる 1 通」であることに価値があるので、安易に割らない。900 に達したら、まず**付録（§C SPDX / §D チェックリスト / §E 追補）を別 file へ出す**ことを検討する —— 送る本文（§A / §B）は1 つに保つ |
