...
| `LICENSES/ACD-1.0.submission.md` | 806 | 900 | `advisory` | OSI / SPDX へ送る提出パケット本体。**2026-09-06 に 800 行を越え Check 454 が発火して登録された** —— 早期警告の無い file を作らないための登録義務ライン。advisory は BLOCKING (1000) の 100 行手前に置く。**分割の disposition**: この file は「そのまま送れる 1 通」であることに価値があるので、安易に割らない。900 に達したら、まず**付録（§C SPDX / §D チェックリスト / §E 追補）を別 file へ出す**ことを検討する —— 送る本文（§A / §B）は1 つに保つ |
| `.github/scripts/checks_license_dossier.py` | 586 | 600 | `advisory` | ACD-1.0 ドシエ (LICENSES/) の自己整合 Check 群（投稿先 458 / 到達性 459 / **入口の言語 459b** / 自己申告件数 460 / 日付の鮮度 461b）。**2026-09-05 に `checks_governance_sync.py` が 966 行で advisory を鳴らしたので切り出した** —— 圧縮で黙らせず、いま触っている塊を出す。ドシエ側の Check は OSI 審査の往復が続くかぎり育つので、800 の登録義務ラインより手前の 600 で早期警告する |
...
.github/scripts/checks_license_dossier.py | 600 | advisory
