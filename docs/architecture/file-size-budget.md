...
| `LICENSES/ACD-1.0.review-precedents.md` | 512 | 900 | `advisory` | `license-review` / `license-discuss` の記録から読み取った、ACD-1.0 に当たる指摘と当たらない指摘（§1.45〜§1.50）。**2026-09-08 に `comparison.md` が advisory を越えたので切り出した** —— 読者も用途も違う（前者は「どれを選ぶべきか」、本書は「この提出は何を言われるか」）。**節番号は変えていない**ので既存の `§1.4x` 参照はそのまま解決する。advisory は BLOCKING (1000) の 100 行手前 |
