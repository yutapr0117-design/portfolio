...
|---|---:|---:|---|---|
| `main.js` | 6,360 | 6,400 | `strong-advisory` | 減少方向。新規機能を直接追加しない。次の縮小は Stage 4/5（baseline 後） |
...
予算（上限）は現行行数より少し上に置いてある。これは「いまの行数は許容範囲内であり、ここから大きく増やすな」という意図の表現である。特に `main.js` は現行 6,360 に対し上限 6,400 と tight に設定し、増加余地を意図的に小さくしている。`main.js` がこの上限に近づいた場合、それは「Stage 4/5 の物理分割で縮小すべき時期が来た」というシグナルである。
