...
| `LICENSES/ACD-1.0.comparison.md` | 695 | 900 | `advisory` | 既存ライセンスとの逐条比較、および**外部レビューの実例から還元した節**（§1.45 AI-MIT / §1.46 PBZC / §1.47 MIT-I / §1.48 Modified 0BSD）。**2026-09-07 に 800 行を越え Check 454 が発火して登録された。** advisory は BLOCKING (1000) の 100 行手前。**分割の disposition**: 「既存ライセンスとの比較」と「審査アーカイブからの還元」は読者も用途も違うので、次に越えたら §1.45〜§1.48 を `ACD-1.0.review-precedents.md` へ切り出す。今切らないのは、still 1 テーマ（「他の instrument に照らして ACD はどこに立つか」）として読めるうちは分割が索引と相互参照を増やすだけだから |
...
| `LICENSES/PEER-REVIEW-WATCH.md` | 807 | 900 | `advisory` | 同時代 instrument（OpenMDW / ModelGo）の経過観察と、**アーカイブ取得の手順・取得済みの窓・そこで得たもの**。2026-09-15 に両アーカイブの全期間取得を実測したので、**窓は取得の制約ではなく「何を測ったか」の記録**になった |
| `LICENSES/ACD-1.0.reviewer-positions.md` | 798 | 900 | `advisory` | 審査者が**主題**について述べたことの記録（§1.49 / §1.50 / §1.52 / §1.55 / §1.56 / §1.58〜§1.61）。**2026-09-09 に `review-precedents.md` を二分して作った** —— 境界は「1 つの提出の顛末か、複数の提出を横断する主題か」。**節番号は変えていない**ので既存参照は解決する。**分割の disposition**: 次に越えたら**条文の主題（人格権・特許・データベース権）と提出の作法（長さ・弁護士・構造）**で割る —— 前者は条文を読みに来た審査者、後者は提出の形を見に来た審査者が読む |
...
| `.github/scripts/checks_license_dossier.py` | 699 | 800 | `advisory` | ACD-1.0 ドシエ (LICENSES/) の自己整合 Check 群（投稿先 458 / 到達性 459 / **入口の言語 459b** / 自己申告件数 460 / 日付の鮮度 461b）。**2026-09-05 に `checks_governance_sync.py` が 966 行で advisory を鳴らしたので切り出した** —— 圧縮で黙らせず、いま触っている塊を出す。ドシエ側の Check は OSI 審査の往復が続くかぎり育つので、800 の登録義務ラインより手前で早期警告する。**2026-09-06 に 700 へラチェット** —— 超過分は Check 463（送る文面が OSI の要求 10 項目を含む）の追加そのもので、コメントを削っても収まらない ＝**冗長さではなく機構の分**。上げた以上は次に鳴ったとき「また上げる」でなく分割を検討する |
...
| `.github/scripts/checks_license_self_reporting.py` | 681 | 700 | `advisory` | Check 460（ドシエの自己申告件数）と Check 469（自己申告の**列挙**が実装と一致すること）を持つ split module。**面の数はここに書かない** —— 実測 2026-09-13 に本欄が「12 面」、module の docstring inventory が「(a)〜(k) の 11 面」、実装が **16 面**と、3 つの数が 3 つとも違っていた。**単一ソースは module の docstring inventory で、Check 469 (a) がそれを実装と双方向照合する。****2026-09-09 に `checks_license_dossier.py` が advisory (800) を越えたので切り出した** —— face (l) を足した増分の中で、圧縮せず「いま触っているクラスタ」を出す方針（`checks_mutation_integrity.py` を `checks_maintainability.py` から切り出したのと同じ手）。**分割の disposition**: 面は増える一方なので、次に advisory を越えたら**入口ページ面 (g)(h) と本文の数値面 (a)〜(f)** で割る —— 前者は「読者が最初に見るページ」、後者は「文書群の規模」で、drift の起き方が違う |
