...
| `LICENSES/BLIND-SPOTS-LOG.md` | 352 | 900 | `advisory` | **`BLIND-SPOTS.md` の適用の日次ログ**（いつ、どの次元を使い、何が出たか）。**2026-09-23 に分割** ——本体が Check 365 の1,000 行に当たったとき、**圧縮ではなく伸び続ける側を切り出した**（日次ログは増分ごとに必ず伸びるが、方法論は伸びない）。**越えたらまた同じ基準で割る** |
...
| `.github/scripts/checks_license_draft.py` | 290 | 400 | `advisory` | **次版の草案 (`ACD-1.2-DRAFT.txt`) を検査する Check 468 だけを持つ split module**。2026-09-24 に `checks_license_submission.py` が 801 行で advisory (800) を越えたので、**予算を上げずに**「審査者がいま受け取る面」と「まだ誰にも渡していない草案」で割った |
...
| `.github/scripts/checks_license_references.py` | 302 | 400 | `advisory` | **ドシエが書いた矢印が、指した先に在るか**だけを持つ split module（Check 471・5 形）。台帳が「機械強制済」と宣言していた種類を壊して測ったら 7 形のうち 5 形が無防備だった、というのが出自。2026-09-15 に dossier module が advisory を越えたので切り出した |
