...
| `js/aidk-rails.js` | 437 | 550 | `advisory` | Stage 5-l (AIDK Rail) 新設。AIDK Rail 5 IIFE 合体 factory (RouteState/EffectRails/BindingRegistry/ActionDelegator/DiagnosticsRail)。closure-deps = none + 引数注入。命名: PR #37。Stage 5-l は本 entry (AIDK Rail) を指し、PR #33 の Meta Management は changelog 上では 5-k' と呼称（命名衝突を honest dating で記録） |
...
| `.github/scripts/check_repository_consistency.py` | 945 | 950 | `advisory` | **2026-07 の check.py 分割トラックで実 Check ロジックを 53 個の `checks_*.py` module へ ctx 注入で分散済み**（15,913→796 行）。本体は薄い dispatcher（module 読み込み + 自己整合集約 Check 45/70/105 の不動点）に縮小した。ceiling 4,750 は分割前の +headroom 値で現状は大幅な余裕があり緩い（実効的な上限は Check 365 の全非 A テキスト ≤1,000 BLOCKING）。各カテゴリ Check は個別 `checks_*.py` 側の budget で管理する（**2026-08-26 是正**: この記述は長らく宣言だけで、実際には `checks_*.py` は 1 件も BUDGET-DATA に登録されて おらず Check 52 の advisory が構造的に鳴らなかった。危険域に達した module を登録し、Check 454 が「>800 行なら登録必須」を BLOCKING 強制する） |
| `LICENSES/ACD-1.0.review-responses.md` | 437 | 900 | `advisory` | OSI での議論・審査を想定した問答ドシエ（現在の投稿先は `license-discuss`）。**レビューで実際に指摘を受けたら追記する**設計なので、Q&A は今後も増える（＝ append-log 的に伸びる面）。advisory 900 に当たったら**テーマで分割する**（第 1 波=テキストの中身 / 第 2 波=起草の出自・名称・運用 / 以後=実際に受けた指摘）。時系列の log ではないので rotate ではなく主題分割が正しい。2026-08-26 に Check 454 が**この file 自身に発火**して登録された（早期警告の二層設計が意図どおり働いた実例）。**同日中に予告どおりテーマ分割を実施**（総論=本 file / 条項別=`-clauses` / 出自・名称・運用=`-meta`）。警告が出た時点で「後で考える」ではなく即座に分けたので、BLOCKING には一度も当たっていない |
| `.github/scripts/checks_governance_sync.py` | 731 | 950 | `advisory` | 統治文書の同期 Check 群（規範層の defer 理由 436 / venue の単一ソース 458 / ライセンス文書の到達性 459 / **自己申告件数** 460 ほか）。**2026-08-27 に Check 454（危険域 >800 行は登録必須）が本 file 自身へ発火して登録された** —— 同日に作った早期警告が、作った本人の module を最初に捕まえた実例 |
| `LICENSES/ACD-1.0.comparison.md` | 726 | 900 | `advisory` | 既存ライセンスとの逐条比較、および**外部レビューの実例から還元した節**（§1.45 AI-MIT / §1.46 PBZC / §1.47 MIT-I / §1.48 Modified 0BSD）。**2026-09-07 に 800 行を越え Check 454 が発火して登録された。** advisory は BLOCKING (1000) の 100 行手前。**分割の履歴**: §1.45〜§1.50 は 2026-09-08 に `ACD-1.0.review-precedents.md` へ（この欄はその後も「次に越えたら切り出す」と書いたまま古くなっていた）。**§1.96 / §1.97 / §1.104（gap の主張を本文に当てた測定）は 2026-09-25 に 893 行・advisory の手前で `ACD-1.0.gap-measurements.md` へ**切り出した。**次の候補**: §1.7（承認済みの「留保」・約 230 行）—— 条項比較の中で唯一「reach の限界」という別の問いを扱う |
| `LICENSES/ACD-1.0.review-precedents.md` | 724 | 900 | `advisory` | `license-review` / `license-discuss` の記録から読み取った、ACD-1.0 に当たる指摘と当たらない指摘（切り出し時点で §1.45〜§1.50、以後 append）。**2026-09-08 に `comparison.md` が advisory を越えたので切り出した** —— 読者も用途も違う（前者は「どれを選ぶべきか」、本書は「この提出は何を言われるか」）。**節番号は変えていない**ので既存の `§1.4x` 参照はそのまま解決する。advisory は BLOCKING (1000) の 100 行手前 |
| `LICENSES/ACD-1.0.review-doctrine.md` | 254 | 800 | `advisory` | リスト上で論じられた法理を **ACD-1.0 の特定の条へ当てた**もの（§1.84 / §1.100 / §1.101 / §1.109）。**2026-09-25 に `review-precedents.md` が 955 行で advisory (900) を越えたので切り出した。** **割る線は行数ではなく主題** —— 残した側は「他の提出に何が起きたか」（撤回・却下・承認・改名・議決）、本書は「その法理を我々の条に当てるとどうなるか」で、読み手の用途が違う。**節番号は変えていない**ので `§1.84` 等の参照は番号として解決し、file 名を添えた参照は向け直した |
| `LICENSES/ACD-1.0.review-responses-clauses.md` | 648 | 900 | `advisory` | **条項別の想定問答**（§1〜§16 を条ごとに、審査者が突いてくる順で並べる）。**2026-09-14 に Check 454 が「800 行を越えたのに予算が無い＝早期警告が存在しない」と検出して初めて登録した** —— それまで**この file だけ無登録**で、advisory も BLOCKING も効いていなかった。**審査の往復が続くかぎり育つ側の file**（1 つの指摘が 1 つの問答になる）。**次に advisory が鳴ったら、条の範囲で割る**（§1〜§9 / §10〜§16）—— テーマではなく**条番号**で割るのは、読み手が「どの条の話か」で探すからである |
| `LICENSES/ACD-1.0.review-responses-boilerplate.md` | 360 | 700 | `advisory` | **想定問答の第 2 層**（Q22〜Q33・定型条項と解釈条項への攻撃）。2026-09-20 に条項別の分冊が advisory を超えたので、既にあった第 1 層 / 第 2 層の境界で切り出した。**Q 番号は動かしていない** ——他文書は番号で参照するので、振り直せば参照だけが静かに壊れる |
| `LICENSES/ACD-1.0.review-corpus.md` | 874 | 900 | `advisory` | **`license-review` / `license-discuss` のアーカイブ全体を取得して測ったこと**（承認の基準 8 条・委員会が述べた規則・決議の分布・提出から決定までの所要・手続きの実態）。**2026-09-13 に `review-precedents.md` が Check 365 の 1,000 行に当たったので切り出した** —— **圧縮ではなくテーマ分割**で、**節番号は動かしていない**（既存参照を保つため）。**新しい節の行き先の規則**: 個別スレッドの読み → `review-precedents.md` / アーカイブ全体の測定 → 本 file |
| `LICENSES/ACD-1.0.review-rules.md` | 472 | 700 | `advisory` | **OSI が公表しているページを原典で読んだ記録**（§1.70 承認基準 8 条 / §1.76 要件リスト / §1.81 CoC 棚卸し / §1.82 本文変更の手続き / §1.86 CoC 逐条）。2026-09-15 に corpus が 1,000 行の BLOCKING に当たったので、**圧縮せず資料の種類で切り出した** —— 公表ページは規則を、リストは議論を、理事会は決定を残す |
| `LICENSES/BLIND-SPOTS.md` | 783 | 900 | `advisory` | **自分の欠陥を探すのに使った次元と、まだ使っていない次元の記録**。**盲点は「まだ探していない場所」ではなく「まだ使っていない測り方」に在る**という観察が中心で、増分ごとに 1 節ずつ伸びる。**2026-09-20 に 800 行を超えて Check 454 が「早期警告が存在しない」と止めた** ——予算が無い file は advisory にかからず、**OK からいきなり 1,000 行の BLOCKING へ飛ぶ**。次に advisory へ当たったら、使い終わった次元の記録を主題で切り出す |
| `LICENSES/BLIND-SPOTS-LOG.md` | 356 | 900 | `advisory` | **`BLIND-SPOTS.md` の適用の日次ログ**（いつ、どの次元を使い、何が出たか）。**2026-09-23 に分割** ——本体が Check 365 の1,000 行に当たったとき、**圧縮ではなく伸び続ける側を切り出した**（日次ログは増分ごとに必ず伸びるが、方法論は伸びない）。**越えたらまた同じ基準で割る** |
| `LICENSES/ACD-1.0.board-decisions.md` | 430 | 700 | `advisory` | **OSI 理事会の決定**（議事録という一次資料からの分析）。**2026-09-13 に `review-corpus.md` が 908 行で advisory (900) を越えたので切り出した** —— 圧縮せず、いま触っている塊を出す方針。**分ける基準は「資料の種類」** ——リストのアーカイブ全体の測定は `review-corpus.md`、個別スレッドの読みは `review-precedents.md`、**理事会という別の発話主体の決定**はここ。**同じ出来事について、リストは議論を、理事会は決定を残す。** 公開議事録は毎月増えるので育つ側の file である |
| `LICENSES/ACD-OSI-BOTTLENECKS.md` | 419 | 700 | `advisory` | **承認を阻害する要因の唯一の canonical 一覧**（B1〜B14 + 2026-09-11 の掘削結果）。**2026-09-11 に 800 行を越え Check 454 が「早期警告が存在しない」と検出したので登録した** —— **黙らせるためではなく、BLOCKING (1,000) の手前で鳴らすため**に advisory を 900 に置く。越えたら**圧縮ではなくテーマ分割**で対処する（分析の切り出し先は `ACD-1.0.review-precedents.md`） **2026-09-13 に advisory を 900 → 950 へ上げた。理由を測ってから上げている** —— 当日の追加は B4 / B5 / B11 への「解消したときに失うもの」欄 **計 54 行**で、**それを除くと 860 行＝予算内**。つまり**超過は冗長さではなく分析の分**である。**圧縮も検討して退けた**: 冒頭 30 行の掘削サマリは「register の各項目はこれを前提に読む」という読み順を与えており、20 行を買うために削るのは**圧縮で黙らせる**ことにあたる。**分割も退けた** —— 本文が「**この文書が唯一の canonical なボトルネック一覧である**」と宣言しており、**割ると単一ソース性そのものを壊す**。**次に鳴ったときの分割線は決めてある**: **索引 + status（短い面）** と **bottleneck ごとの深い分析（長い面）** で割る |
| `LICENSES/ACD-OSI-BOTTLENECKS-EXTERNAL.md` | 701 | 800 | `advisory` | **register のうち「外の答えに依存する 7 項目」の深い分析**（B1+B2 / B4 / B5 / B7 / B10 / B11 / B14）。**2026-09-14 に register が 957 行で advisory (950) を鳴らしたので分割した。** **⚠ 分割線は前日に測らずに書いた線とは違う** —— 「索引+status / 深い分析」で割ると **85 / 871** で受け皿が即座に再び鳴る。**測ってから「行為主体で割る」へ変えた（370 / 586）。** **索引・分類・集計は分離しない** ——本文が「唯一の canonical なボトルネック一覧」と宣言しており、一覧＝索引の表だから |
| `LICENSES/PEER-REVIEW-WATCH.md` | 888 | 900 | `advisory` | 同時代 instrument（OpenMDW / ModelGo）の経過観察と、**アーカイブ取得の手順・取得済みの窓・そこで得たもの**。2026-09-15 に両アーカイブの全期間取得を実測したので、**窓は取得の制約ではなく「何を測ったか」の記録**になった |
| `LICENSES/ACD-1.0.reviewer-positions.md` | 634 | 900 | `advisory` | 審査者が**主題**について述べたことの記録（§1.49 / §1.50 / §1.52 / §1.55 / §1.56 / §1.58〜§1.61）。**2026-09-09 に `review-precedents.md` を二分して作った** —— 境界は「1 つの提出の顛末か、複数の提出を横断する主題か」。**節番号は変えていない**ので既存参照は解決する。**分割の disposition**: 次に越えたら**条文の主題（人格権・特許・データベース権）と提出の作法（長さ・弁護士・構造）**で割る —— 前者は条文を読みに来た審査者、後者は提出の形を見に来た審査者が読む |
| `LICENSES/ACD-1.0.review-labels.md` | 204 | 500 | `advisory` | **リストが我々の類型に付ける呼び名の記録（§1.98 クレヨン / §1.99 vanity）。**2026-09-19 に `reviewer-positions.md` が 1,000 行に当たったので切り出した —— 境界は「**特定の人・特定の提出についての記録か、呼び名についての記録か**」。**節番号は変えていない**ので既存参照は解決する。**分割の disposition**: 次に越えたら**呼び名ごと**に割る（呼び名は互いに独立で、読む動機も別々である）|
| `LICENSES/ACD-1.0.objection-map.md` | 99 | 300 | `advisory` | 既知の反論が ACD-1.0 に当たるかを 1 表にした文書（§1.62 のみ）。**2026-09-09 に `review-precedents.md` が advisory を越えたので切り出した** —— ただし理由は規模だけではない。**他の節の合成であって precedent の記録ではなく、審査者が最初に読むべきもの**なので、900 行の末尾に置いておくのは #6（長さ）への数少ない実務的対処を捨てることになる。**節番号 §1.62 は変えていない**ので既存の参照は解決する。**分割の disposition**: 表が 2 つ以上になったら「当たる」「当たらない」で割らない —— **並べて読めることがこの文書の唯一の機能**である |
| `LICENSES/ACD-1.0.submission.md` | 551 | 700 | `advisory` | OSI / SPDX へ送る提出パケット本体（§A 要求情報 / §B.0 送る本文 / §C SPDX / §D 送信前チェック / §E 追補）。**2026-09-06 に 800 行を越え Check 454 が発火して登録された。** **2026-09-09 に 976 行で advisory を越え、§B.1 の参考資料（§1〜§5・約 580 行）を `ACD-1.0.submission-reference.md` へ切り出した。** **⚠ これは本欄が事前に宣言していた分割計画とは違う。** 旧計画は「900 に達したら付録（§C / §D / §E）を出す。送る本文（§A / §B）は 1 つに保つ」だった。**旧計画の前提が不正確だった** —— §B を 1 つの塊として扱っていたが、**§B.0 は送る人が読む本文、§B.1 は審査者が読む参考資料**で、読み手が違う。付録 112 行を出せば 864 行になり数字の上では収まるが、**貼ってはいけない 580 行が送る本文と同居したまま**になる（§B.0 は「§1〜§5 を貼るな」と 3 度書いていた＝同居している限り必要な警告）。**事前計画から外れたので、黙って外れずにここへ書く。** 新 advisory は 700（BLOCKING 1000 の 300 行手前）—— 切り出し後 390 行なので、次に越えたときは付録を出す旧計画がそのまま使える |
| `LICENSES/ACD-1.0.submission-reference.md` | 764 | 900 | `advisory` | 提出文の背後にある参考資料（§1 なぜ新しいライセンスが要るか / §2 既存との差 / §3〜§3d は案内見出しのみ（本体は `ACD-1.0.submission-osd.md`）/ §4 意図的な不在 / §4a トラック / §4b 提出者専用でないこと / §4c 機械的検証 / §5 開示）。**2026-09-09 に `submission.md` から切り出した。** **節番号は変えていない**ので既存の `submission.md §4c` 等の参照は本書の同じ節へ解決する（`comparison.md` → `review-precedents.md` と同じ手）。**分割の disposition**: 宣言どおり **§3 系（OSD 逐条・§3b・§3c・§3d）を 2026-09-25 に `ACD-1.0.submission-osd.md` へ切り出した** —— **advisory を越える前に**（899 行・残り 1 行の時点で）。切り出し元に案内見出しを残したので、他文書の `submission-reference.md §3b` 等 21 参照は張り替えていない。次に越えたら **§4 系（§4〜§4c）** が同じ理由（「提出者専用でないか・機械検証」という単一の問いで読まれる）で候補 |
| `LICENSES/ACD-1.1-CHANGELIST.md` | 818 | 900 | `advisory` | **次版（1.1 草案）の変更記録** —— errata 由来の修正・縮小候補の実測・外部レビュー由来の角度・**この草案が自分で持ち込んだ欠陥（§3e）**。**2026-09-17 に 800 行を越え Check 454 が「早期警告が存在しない」と検出したので登録した。** **草案が動くかぎり育つ側の file** —— 1 つの改訂が 1 つの記録になる。**分割の disposition**: 次に advisory が鳴ったら **「本文をどう変えたか」（§0〜§2）と「外から来た材料を当てた結果」（§3 系）**で割る —— 前者は次の起草者が、後者は審査者が読む。**圧縮では対処しない** ——この file の行は大半が「なぜそう変えたか」で、削ると次の起草者が同じ検討をやり直す |
| `LICENSES/ACD-1.2-DRAFT.txt` | 804 | 900 | `advisory` | **次版の草案**（ヘッダの変更一覧 + 本文 80 条）。ヘッダは変更 1 件ごとに理由まで書くので伸びる。**本文の語数は register B3 の費用として別に数える**（Check 468c が申告と実測の一致を強制）。900 を越えたら、ヘッダの理由づけを `ACD-1.2-CHANGELIST.md` へ寄せて短くする |
| `.github/scripts/checks_license_dossier.py` | 578 | 800 | `advisory` | ACD-1.0 ドシエ (LICENSES/) の自己整合 Check 群（投稿先 458 / 到達性 459 / **入口の言語 459b** / 自己申告件数 460 / 日付の鮮度 461b）。**2026-09-05 に `checks_governance_sync.py` が 966 行で advisory を鳴らしたので切り出した** —— 圧縮で黙らせず、いま触っている塊を出す。ドシエ側の Check は OSI 審査の往復が続くかぎり育つので、800 の登録義務ラインより手前で早期警告する。**2026-09-06 に 700 へラチェット** —— 超過分は Check 463（送る文面が OSI の要求 10 項目を含む）の追加そのもので、コメントを削っても収まらない ＝**冗長さではなく機構の分**。上げた以上は次に鳴ったとき「また上げる」でなく分割を検討する |
| `.github/scripts/checks_license_submission.py` | 567 | 800 | `advisory` | **審査者が受け取る面だけを検査する Check 群**（463 送る文面 / 468 次版草案 / 472 条項引用 / 473 来歴の開示）。**2026-09-22 に `checks_license_dossier.py` が 914 行で advisory (800) を越えたので切り出した** ——**圧縮ではなくテーマ分割**。**分ける基準は「対象が誰の手に渡るか」**で、残した側の損害は「記録が古い」、こちら側は「**審査者が誤った物を読む**」。越えたらまた同じ基準で割る |
| `.github/scripts/checks_license_draft.py` | 306 | 400 | `advisory` | **次版の草案 (`ACD-1.2-DRAFT.txt`) を検査する Check 468 だけを持つ split module**。2026-09-24 に `checks_license_submission.py` が 801 行で advisory (800) を越えたので、**予算を上げずに**「審査者がいま受け取る面」と「まだ誰にも渡していない草案」で割った |
| `.github/scripts/checks_license_quotation.py` | 143 | 500 | `advisory` | **引用符の中身が帰属先に実在すること**だけを持つ split module（Check 470）。外部 source の引用は `verify_dossier_quotations.py`（外部 fetch ゆえ CI 非搭載）が持ち、本 module は**リポジトリ内に凍結された ACD-1.0 本文**だけを見るので offline で決定的。2026-09-14 に dossier module が advisory を越えたので切り出した |
| `.github/scripts/checks_license_references.py` | 313 | 400 | `advisory` | **ドシエが書いた矢印が、指した先に在るか**だけを持つ split module（Check 471・5 形）。台帳が「機械強制済」と宣言していた種類を壊して測ったら 7 形のうち 5 形が無防備だった、というのが出自。2026-09-15 に dossier module が advisory を越えたので切り出した |
| `.github/scripts/checks_license_self_reporting.py` | 865 | 800 | `advisory` | Check 460（ドシエの自己申告件数）と Check 469（自己申告の**列挙**が実装と一致すること）を持つ split module。**面の数はここに書かない** —— 実測 2026-09-13 に本欄が「12 面」、module の docstring inventory が「(a)〜(k) の 11 面」、実装が **16 面**と、3 つの数が 3 つとも違っていた。**単一ソースは module の docstring inventory で、Check 469 (a) がそれを実装と双方向照合する。****2026-09-09 に `checks_license_dossier.py` が advisory (800) を越えたので切り出した** —— face (l) を足した増分の中で、圧縮せず「いま触っているクラスタ」を出す方針（`checks_mutation_integrity.py` を `checks_maintainability.py` から切り出したのと同じ手）。**分割の disposition**: 面は増える一方なので、次に advisory を越えたら**入口ページ面 (g)(h) と本文の数値面 (a)〜(f)** で割る —— 前者は「読者が最初に見るページ」、後者は「文書群の規模」で、drift の起き方が違う  **2026-09-22: 732 行で advisory (700) を越えた。予算は上げない** ——規律は「全部消しても収まらない＝超過は機能の分」を確認してから上げることで、**実測するとコメントを全部消せば 599 行で収まる**＝条件を満たさない。**だが圧縮もしない**: 各 face の WHY は「その face がなぜ在るか」の唯一の記録で、消せば次のセッションが同じ drift を再発見する。**次に鳴ったときの分割線を先に決めておく** ——**Check 460（数の申告）と Check 469（列挙の申告）で割る**（469 は約 90 行で独立している）。**先に決めるのは、改訂の圧力下で決めると都合よく割るから。**| **⚠ 2026-09-22 に予定していた分割を実行し、接触で反証されたので撤回した。** 先に決めてあった分割線は「Check 460（数の申告）/ Check 469（列挙の申告）」だったが、**切り出すと Check 469 が自分の仕事をできなくなる** ——469 は **`checks_license_self_reporting.py` の docstring から Check 460 の face 列挙を取り出して照合する**ので、469 の entry を抜くと**列挙ブロックの終端子が消えて抽出が失敗する**（実測: 抽出直後に RED）。抽出した block が他 module の局所変数 `_L460` を参照して `NameError` で suite ごと落ちもした（**握り潰されず止まったのは Check 400 の設計どおり**）。**この file には切れる継ぎ目が 1 つしか無く、その 1 つが結合している。** **したがって予算を 700 → 800 へ上げる。理由は「コメントが長い」ではなく「分割できないことを実測した」である** ——**上げる前に分割を試して失敗した記録がこの行の根拠**。BLOCKING (1,000) まで 269 行の余裕がある。**次に鳴ったら、分割ではなく Check 460 の face そのものを減らせるかを問うこと**（face は事故のたびに 1 つずつ足されており、統合できる組が在りうる）。
...
| `.github/scripts/checks_aio_config.py` | 908 | 950 | `advisory` | AIO 設定面の Check 群。同上の cohort |
...
| `.github/scripts/mutation_samples_e2e_archive3.py` | 765 | 975 | `advisory` | behavior mutation の 3 番目の archive。archive2 が 1,000 行上限に達したため 2 段構成へ（archive も無限には伸ばせない） |
...
| `e2e/quiz-lazy-load.spec.js` | 341 | 900 | `advisory` | behavior e2e spec (quiz データの遅延読み込み契約)。#1239 で 130,595 bytes をクリティカルパスから外した結果**「まだ届いていない」状態が新しく生まれた**ため、その扱いを 1 箇所へ集約した。2026-08-23 に quiz.spec.js が 923 行で advisory(900) を超えたため BLOCKING の手前で切り出し |
...
.github/scripts/checks_license_dossier.py | 800 | advisory
.github/scripts/checks_license_quotation.py | 500 | advisory
.github/scripts/checks_license_references.py | 400 | advisory
.github/scripts/checks_license_self_reporting.py | 800 | advisory
LICENSES/ACD-1.0.comparison.md | 900 | advisory
LICENSES/ACD-1.0.review-precedents.md | 900 | advisory
LICENSES/ACD-1.0.review-doctrine.md | 800 | advisory
LICENSES/ACD-1.0.review-responses-clauses.md | 900 | advisory
LICENSES/ACD-1.0.review-responses-boilerplate.md | 700 | advisory
LICENSES/ACD-1.0.review-corpus.md | 900 | advisory
LICENSES/ACD-1.0.review-rules.md | 700 | advisory
LICENSES/BLIND-SPOTS.md | 900 | advisory
LICENSES/BLIND-SPOTS-LOG.md | 900 | advisory
LICENSES/ACD-1.0.board-decisions.md | 700 | advisory
LICENSES/ACD-OSI-BOTTLENECKS.md | 700 | advisory
LICENSES/ACD-OSI-BOTTLENECKS-EXTERNAL.md | 800 | advisory
LICENSES/PEER-REVIEW-WATCH.md | 900 | advisory
LICENSES/ACD-1.0.reviewer-positions.md | 900 | advisory
LICENSES/ACD-1.0.review-labels.md | 500 | advisory
LICENSES/ACD-1.0.objection-map.md | 300 | advisory
LICENSES/ACD-1.0.submission.md | 700 | advisory
LICENSES/ACD-1.0.submission-reference.md | 900 | advisory
LICENSES/ACD-1.1-CHANGELIST.md | 900 | advisory
LICENSES/ACD-1.2-DRAFT.txt | 900 | advisory
