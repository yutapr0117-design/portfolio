...
| `LICENSES/BLIND-SPOTS.md` | 919 | 900 | `advisory` | **自分の欠陥を探すのに使った次元と、まだ使っていない次元の記録**。**盲点は「まだ探していない場所」ではなく「まだ使っていない測り方」に在る**という観察が中心で、増分ごとに 1 節ずつ伸びる。**2026-09-20 に 800 行を超えて Check 454 が「早期警告が存在しない」と止めた** ——予算が無い file は advisory にかからず、**OK からいきなり 1,000 行の BLOCKING へ飛ぶ**。次に advisory へ当たったら、使い終わった次元の記録を主題で切り出す |
...
| `.github/scripts/checks_license_submission.py` | 455 | 800 | `advisory` | **審査者が受け取る面だけを検査する Check 群**（463 送る文面 / 468 次版草案 / 472 条項引用 / 473 来歴の開示）。**2026-09-22 に `checks_license_dossier.py` が 914 行で advisory (800) を越えたので切り出した** ——**圧縮ではなくテーマ分割**。**分ける基準は「対象が誰の手に渡るか」**で、残した側の損害は「記録が古い」、こちら側は「**審査者が誤った物を読む**」。越えたらまた同じ基準で割る |
