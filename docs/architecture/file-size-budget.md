...
| `LICENSES/ACD-1.0.comparison.md` | 725 | 900 | `advisory` | 既存ライセンスとの逐条比較、および**外部レビューの実例から還元した節**（§1.45 AI-MIT / §1.46 PBZC / §1.47 MIT-I / §1.48 Modified 0BSD）。**2026-09-07 に 800 行を越え Check 454 が発火して登録された。** advisory は BLOCKING (1000) の 100 行手前。**分割の履歴**: §1.45〜§1.50 は 2026-09-08 に `ACD-1.0.review-precedents.md` へ（この欄はその後も「次に越えたら切り出す」と書いたまま古くなっていた）。**§1.96 / §1.97 / §1.104（gap の主張を本文に当てた測定）は 2026-09-25 に 893 行・advisory の手前で `ACD-1.0.gap-measurements.md` へ**切り出した。**次の候補**: §1.7（承認済みの「留保」・約 230 行）—— 条項比較の中で唯一「reach の限界」という別の問いを扱う |
...
| `LICENSES/BLIND-SPOTS-LOG.md` | 304 | 900 | `advisory` | **`BLIND-SPOTS.md` の適用の日次ログ**（いつ、どの次元を使い、何が出たか）。**2026-09-23 に分割** ——本体が Check 365 の1,000 行に当たったとき、**圧縮ではなく伸び続ける側を切り出した**（日次ログは増分ごとに必ず伸びるが、方法論は伸びない）。**越えたらまた同じ基準で割る** |
