...
| `LICENSES/ACD-1.0.review-corpus.md` | 855 | 900 | `advisory` | **`license-review` / `license-discuss` のアーカイブ全体を取得して測ったこと**（承認の基準 8 条・委員会が述べた規則・決議の分布・提出から決定までの所要・手続きの実態）。**2026-09-13 に `review-precedents.md` が Check 365 の 1,000 行に当たったので切り出した** —— **圧縮ではなくテーマ分割**で、**節番号は動かしていない**（既存参照を保つため）。**新しい節の行き先の規則**: 個別スレッドの読み → `review-precedents.md` / アーカイブ全体の測定 → 本 file |
| `LICENSES/ACD-1.0.review-rules.md` | 407 | 700 | `advisory` | **OSI が公表しているページを原典で読んだ記録**（§1.70 承認基準 8 条 / §1.76 要件リスト / §1.81 CoC 棚卸し / §1.82 本文変更の手続き / §1.86 CoC 逐条）。2026-09-15 に corpus が 1,000 行の BLOCKING に当たったので、**圧縮せず資料の種類で切り出した** —— 公表ページは規則を、リストは議論を、理事会は決定を残す |
...
| `LICENSES/PEER-REVIEW-WATCH.md` | 868 | 900 | `advisory` | 同時代 instrument（OpenMDW / ModelGo）の経過観察と、**アーカイブ取得の手順・取得済みの窓・そこで得たもの**。2026-09-15 に両アーカイブの全期間取得を実測したので、**窓は取得の制約ではなく「何を測ったか」の記録**になった |
