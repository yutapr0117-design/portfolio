...
Track         : v80+ staged major update (Phase 2 — Check 78/80 name==identifier coherence: check_repository_consistency.py 行数 4,192 / budget 4,400・_lib_io.py 217 / 250 へ同期)
...
| `.github/scripts/check_repository_consistency.py` | 4,192 | 4,400 | `advisory` | 中央 enforcement registry ゆえ Check 追加ごとに約 35 行/件で構造的に成長する設計 (Check 100-109 + 102d + 76/78/80 拡張 等を順次追加)。budget は実態 +headroom (約 5 件分) へ同期。これは「抑制すべき bloat」ではなく「機械強制の richness 増加」ゆえ ceiling は緩やかに追従 |
