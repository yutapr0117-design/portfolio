...
| `LICENSES/ACD-1.0.review-responses-boilerplate.md` | 339 | 700 | `advisory` | **想定問答の第 2 層**（Q22〜Q33・定型条項と解釈条項への攻撃）。2026-09-20 に条項別の分冊が advisory を超えたので、既にあった第 1 層 / 第 2 層の境界で切り出した。**Q 番号は動かしていない** ——他文書は番号で参照するので、振り直せば参照だけが静かに壊れる |
...
| `LICENSES/ACD-1.0.review-rules.md` | 319 | 700 | `advisory` | **OSI が公表しているページを原典で読んだ記録**（§1.70 承認基準 8 条 / §1.76 要件リスト / §1.81 CoC 棚卸し / §1.82 本文変更の手続き / §1.86 CoC 逐条）。2026-09-15 に corpus が 1,000 行の BLOCKING に当たったので、**圧縮せず資料の種類で切り出した** —— 公表ページは規則を、リストは議論を、理事会は決定を残す |
...
| `LICENSES/ACD-OSI-BOTTLENECKS-EXTERNAL.md` | 838 | 800 | `advisory` | **register のうち「外の答えに依存する 7 項目」の深い分析**（B1+B2 / B4 / B5 / B7 / B10 / B11 / B14）。**2026-09-14 に register が 957 行で advisory (950) を鳴らしたので分割した。** **⚠ 分割線は前日に測らずに書いた線とは違う** —— 「索引+status / 深い分析」で割ると **85 / 871** で受け皿が即座に再び鳴る。**測ってから「行為主体で割る」へ変えた（370 / 586）。** **索引・分類・集計は分離しない** ——本文が「唯一の canonical なボトルネック一覧」と宣言しており、一覧＝索引の表だから |
| `LICENSES/PEER-REVIEW-WATCH.md` | 846 | 900 | `advisory` | 同時代 instrument（OpenMDW / ModelGo）の経過観察と、**アーカイブ取得の手順・取得済みの窓・そこで得たもの**。2026-09-15 に両アーカイブの全期間取得を実測したので、**窓は取得の制約ではなく「何を測ったか」の記録**になった |
