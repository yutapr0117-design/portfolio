...
| `LICENSES/ACD-1.0.board-decisions.md` | 327 | 700 | `advisory` | **OSI 理事会の決定**（議事録という一次資料からの分析）。**2026-09-13 に `review-corpus.md` が 908 行で advisory (900) を越えたので切り出した** —— 圧縮せず、いま触っている塊を出す方針。**分ける基準は「資料の種類」** ——リストのアーカイブ全体の測定は `review-corpus.md`、個別スレッドの読みは `review-precedents.md`、**理事会という別の発話主体の決定**はここ。**同じ出来事について、リストは議論を、理事会は決定を残す。** 公開議事録は毎月増えるので育つ側の file である |
