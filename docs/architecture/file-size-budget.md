...
| `js/settings-io.js` | 345 | 400 | `advisory` | 2026-08-20 bloat-reduction 分離 (js/settings-page.js より・746→483 行)。Settings の入出力 factory（export 4 形 / import の形の正規化 / lossParts）。UI フラグは getter 注入で読み取り専用ゆえ closure を持ち出さない |
...
| `e2e/apps-settings-io.spec.js` | 653 | 900 | `advisory` | behavior e2e spec。Check 365 の 1,000 行 BLOCKING 上限の手前で警告する早期警告層 |
