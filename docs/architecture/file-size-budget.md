...
| `LICENSES/ACD-1.0.submission-reference.md` | 877 | 900 | `advisory` | 提出文の背後にある参考資料（§1 なぜ新しいライセンスが要るか / §2 既存との差 / §3 OSD 逐条 / §3b 反対側 / §3c 却下理由 / §3d 主題適格 / §4 意図的な不在 / §4a トラック / §4b 提出者専用でないこと / §4c 機械的検証 / §5 開示）。**2026-09-09 に `submission.md` から切り出した。** **節番号は変えていない**ので既存の `submission.md §4c` 等の参照は本書の同じ節へ解決する（`comparison.md` → `review-precedents.md` と同じ手）。**分割の disposition**: 次に advisory を越えたら **§3 系（OSD 逐条・§3b・§3c）を切り出す** —— そこだけは「審査者が OSD の話をしに来たとき」という単一の用途で読まれ、他の節と読み手が分かれる |
...
| `.github/scripts/checks_license_submission.py` | 563 | 800 | `advisory` | **審査者が受け取る面だけを検査する Check 群**（463 送る文面 / 468 次版草案 / 472 条項引用 / 473 来歴の開示）。**2026-09-22 に `checks_license_dossier.py` が 914 行で advisory (800) を越えたので切り出した** ——**圧縮ではなくテーマ分割**。**分ける基準は「対象が誰の手に渡るか」**で、残した側の損害は「記録が古い」、こちら側は「**審査者が誤った物を読む**」。越えたらまた同じ基準で割る |
