...
| `LICENSES/ACD-1.0.reviewer-positions.md` | 892 | 900 | `advisory` | 審査者が**主題**について述べたことの記録（§1.49 / §1.50 / §1.52 / §1.55 / §1.56 / §1.58〜§1.61）。**2026-09-09 に `review-precedents.md` を二分して作った** —— 境界は「1 つの提出の顛末か、複数の提出を横断する主題か」。**節番号は変えていない**ので既存参照は解決する。**分割の disposition**: 次に越えたら**条文の主題（人格権・特許・データベース権）と提出の作法（長さ・弁護士・構造）**で割る —— 前者は条文を読みに来た審査者、後者は提出の形を見に来た審査者が読む |
