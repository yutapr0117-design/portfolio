...
## 1. AI が自らに課す三つの禁止（オーナーが課したものではない）

> **⚠ 2026-09-13 に見出しを直した。** 直前の見出しは、これらを **「禁止事項」**と呼び、
> その由来を、オーナーが強い調子で発したものとして書いていた（**旧文の語はここに引き写さない**
> —— すぐ下の **Check 436b** がまさにその言い回しを禁じており、**引用すると自分の注記が違反になる。**
> 本日 Check 450 の除外注記で同じことを 1 度踏んでいる）。**canon に 2 か所で反していた。**
> (1) オーナーは **「禁止事項 0」**と明言しており（2026-08-23・`CLAUDE.md` §7）、
> **禁止を課した主体はオーナーではない。**
> (2) **オーナーは発話を依頼の形でしか行わない**（Request, not command・
> Check 102g が canon 上の presence を、**436b が我々自身の散文**を強制する）。
> **下の三つは、オーナーの方針のもとで AI が自らに課している作業規律である。**
> **由来は依頼、拘束は自前** —— この区別を落とすと、次に読む AI は「人間が禁止した領域がある」と
> 読み、実在しない制限のもとで動く（**それは §7 が名指しする最大の運用逸脱そのものである**）。
