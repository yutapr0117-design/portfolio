...
- **安全ゲート（C5・設計領域）— WCAG 1.4.10 Reflow の 320px 横あふれ（2026-08-10 実測）:** 320×800 で主要 8 ルートを測ったところ、**4 ルートで水平あふれ**が出る（`#/role-split` 51px / `#/quiz` 31px / `#/hiring-risk` 28px / `#/apps/pomodoro` 16px。`#/` `#/projects` `#/apps/task` `#/settings` は 0px）。これは folklore ではなく再測定可能な数値であり、**保留の根拠として記録する**。
  - **あふれの起点は `MAIN.main-content` が viewport より広くなること**（実測 336〜371px）。`box-sizing: border-box` と `min-width: 0` は効いており、末端は普通のテキスト（例 `H1.h2` が 319px）で、**あふれ量がほぼ水平 padding（32px）と一致する**ことから、幅指定と padding の相互作用が疑われる。根因の特定には style.css のレイアウト系の追跡が要る。
  - **機構の特定（2026-08-10 追加調査でここまで確定。次はここから始めてよい）**: `#app` は 320px では `flex-direction: column`（sidebar は `display:none` で正しく畳まれている）。`#main-content` は `flex: 1 1 0%` / `min-width: 0` だが、**column flex では `flex-basis` は高さに効き、幅（cross 軸）は `align-items: stretch` と子の min-content の大きい方になる**。実測でも `#app` は 320px なのに `#main-content` だけ 351px。したがって「flex item が min-width:0 なのになぜ広がるのか」は誤った問い立てで、**cross 軸の min-content が真因**。
    - **切り分け済み（再調査不要）**: (i) `#main-content` 配下で **自分の箱を超える要素は無い**（`scrollWidth > clientWidth` は `.sr-only` のみで、これは 1px にクリップする標準手法ゆえレイアウト幅に寄与しない）。(ii) `min-width` に固定値を持つのは `.quiz-q-badge`（36px）程度で、単独では 351px を説明しない。→ 残る候補は「複数の固定幅・gap・padding の積み上げ」で、`#content.container` 配下を幅の大きい順に辿るのが次の一手。
    - **落とし穴**: この診断に Playwright の `addStyleTag` は使えない（本サイトの CSP が inline style を弾き、テスト自体が落ちる）。候補 CSS を試すなら `element.style.setProperty` を `evaluate` 内で使うか、`style.css` を直接編集して測ること。
  - **保留理由（安全ゲート・C5）**: 修正は `style.css` のレイアウト規則に及び、複数ページの見た目を変える。**視覚 baseline はローカルで再生成できない**（生成は `update-playwright-snapshots.yml` の human-merge 経路のみ・CLAUDE.md §3）ため、AI 側で非破壊性を機械的に証明できない。加えて配色・余白の設計判断は C5（人間）の領域。
  - **例外の可能性**: `#/role-split` は 2 次元の分担表であり、WCAG 1.4.10 は「使用や意味のために 2 次元レイアウトを要する部分」を明示的に除外する。**4 ルートすべてが違反とは限らない**（この切り分けも設計判断を含む）。
  - **適用条件・次の一手**: (1) baseline 再生成の human-merge 経路を通せる時、(2) まず `#/quiz` `#/hiring-risk` `#/apps/pomodoro` の 3 ルート（表ではないので除外規定が使えない）から、(3) 再現は `page.setViewportSize({width:320,height:800})` で `documentElement.scrollWidth - clientWidth` を測る（本記録の数値がそのまま回帰の基準になる）。
