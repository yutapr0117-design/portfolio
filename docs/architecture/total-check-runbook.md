...
| `npm run check` | `Repository consistency check passed — all invariants hold.` / exit 0 / consistency 240 OK 行（`npm run check` 全体＝consistency＋digest＋binary の 3 スクリプトで、`OK:` トークン行は合計 242）| §6 の registry 参照。1 つでも ERROR が出れば exit 1（BLOCKING）。OK 行数の権威値は §9 の実測表。両者がずれた場合は §9 を正とし、本行を §9 に合わせて更新する |
...
| consistency 検査の `OK:` 行 | **240**（`python3 .github/scripts/check_repository_consistency.py` 単体の `^OK:` 行数。2026-06-16「prose count-hardcode drift guard」increment で実測。Check 104〜109 を含む。本行は以前 135 のまま停留し Check 96〜103 等の追加分が未同期だった (OK 行数は機械強制対象外＝verify は緑のまま drift が見えなかった)。本 increment で測定真値へ是正。Check 47 の 9 モジュール × 3 サブチェック等の動的展開を含むため、新 Check 追加 1 件 ≠ OK 行 +1 になる場合がある点に注意）|
| `npm run check` 全体の `OK:` トークン行 | **242**（consistency 240 ＋ `check_binary_aio_metadata.py` 2。`check_aio_digests.py` は `OK (manifest/...)` 形式と末尾 `AIO digest check passed` を出力し、`OK:` トークンには 0 行寄与する。3 スクリプトはいずれも exit 0。2026-06-15 実測）|
