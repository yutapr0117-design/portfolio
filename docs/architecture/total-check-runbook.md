...
- 前提知識: このリポジトリは **AI-only 実装 + 人間オーケストレーション**。コードは AI が書き、人間が設計・レビュー・監査・統制する。**この検証機構（39 個の整合チェック + CI + 本 runbook）が、その運用を安全にしている核**である。検証を省略すると安全性の前提が崩れる。
...
| 指標 | 基準値 |
|---|---|
| 追跡ファイル総数 | 67（本 runbook 追加後は 68）|
| `npm run lint` | 0 errors / 199 warnings（`curly`:124 / `no-var`:64 / `no-shadow`:10 / `prefer-const`:1、すべて `main.js`）|
| `npm run check` OK 行 | 78 / `all invariants hold` |
| consistency Check 総数 | 39（最大番号 39）|
