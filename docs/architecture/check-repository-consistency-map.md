...
Track         : v80+ staged major update (Phase 2 — post-Stage 5 「CI 更なる手厚化 v2」 で Check 62〜71 を追加)
Subject       : .github/scripts/check_repository_consistency.py（≈2,700 行・Check 1〜71）の構造地図
...
| 64 | check-repository-consistency-map.md table 行が `Check 1..N` 昇順（番号衝突防止） | BLOCKING |
...
| 級 | チェック数 | 該当 |
|---|---:|---|
| BLOCKING（失敗で exit 1） | 67 | Check 52/60 を除く全カテゴリのチェック（Check 53〜59, 61〜71 を含む。サブチェックを持つものは全サブが BLOCKING） |
