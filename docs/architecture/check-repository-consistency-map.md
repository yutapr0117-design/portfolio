...
Track         : v80+ staged major update (Phase 2 — 「C6 derived-value 例外」 で Check 91〜95 + canon 改定 + binary 日付同期 + _lib_io date helpers + aio-manifest last_metadata_update)
Subject       : .github/scripts/check_repository_consistency.py（≈3,500 行・Check 1〜95）の構造地図
...
| 級 | チェック数 | 該当 |
|---|---:|---|
| BLOCKING（失敗で exit 1） | 91 | Check 52/60 を除く全カテゴリのチェック（Check 53〜59, 61〜95 を含む。サブチェックを持つものは全サブが BLOCKING） |
