...
Track         : v80+ staged major update (Phase 2 — 「Repo-wide & Claude Code files」 で Check 76〜80 + .claude/{agents,skills}/ + .mcp.json + .editorconfig + .nvmrc 新設)
Subject       : .github/scripts/check_repository_consistency.py（≈3,100 行・Check 1〜80）の構造地図
...
| 級 | チェック数 | 該当 |
|---|---:|---|
| BLOCKING（失敗で exit 1） | 76 | Check 52/60 を除く全カテゴリのチェック（Check 53〜59, 61〜80 を含む。サブチェックを持つものは全サブが BLOCKING） |
