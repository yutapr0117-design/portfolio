...
| 113 | CLAUDE.md §5 が「AI2AI handoff-first commit/PR 規律」（fine commit ×厚い what+why ×テーマ束ね PR ×`gh pr merge --rebase` ×commit 数は OUTPUT）をマーカーで保持。「AI は交換可能なメンバ」軸の AI→AI 引き継ぎルールが turn-1 router から silent に消えて squash/粗 commit へ退行するのを防ぐ | BLOCKING |
...
| 級 | チェック数 | 該当 |
|---|---:|---|
| BLOCKING（失敗で exit 1） | 113 | Check 52/60 を除く全カテゴリのチェック（Check 53〜59, 61〜117 を含む。サブチェックを持つものは全サブが BLOCKING） |
