...
Track         : v80+ staged major update (Phase 2 — 「All-files AIO coherence」 で Check 81〜85 + binary Organization 注入 + .claude/README.md 新設)
Subject       : .github/scripts/check_repository_consistency.py（≈3,250 行・Check 1〜85）の構造地図
...
| 級 | チェック数 | 該当 |
|---|---:|---|
| BLOCKING（失敗で exit 1） | 81 | Check 52/60 を除く全カテゴリのチェック（Check 53〜59, 61〜85 を含む。サブチェックを持つものは全サブが BLOCKING） |
