...
| 122 | private source document（.pdf/.docx/.doc/.xlsx/.pptx）が一切 tracked されない（`git ls-files` 権威）。本人の経歴書類原本は抽象化済み docs/evidence/real-work-claims.md 生成のためのローカル入力に過ぎず、commit すると機微 PII（実名以外の個人特定情報・顧客/案件名・年収・労働条件）漏洩になる。.gitignore のブランケット ignore と二重防御で、明示 add の取りこぼし・将来の再投入を BLOCKING 捕捉 | BLOCKING |
