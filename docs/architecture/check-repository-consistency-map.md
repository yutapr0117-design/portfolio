...
Track         : v80+ staged major update (Phase 2 — post-Stage 5 「All Plans Combined」 で Check 72〜75 + helper 抽出 + incident README 新設)
Subject       : .github/scripts/check_repository_consistency.py（≈2,900 行・Check 1〜75）の構造地図
...
| 級 | チェック数 | 該当 |
|---|---:|---|
| BLOCKING（失敗で exit 1） | 71 | Check 52/60 を除く全カテゴリのチェック（Check 53〜59, 61〜75 を含む。サブチェックを持つものは全サブが BLOCKING） |
