...
// ===== リセットの結果が見えない利用者にも届くこと (WCAG 4.1.3) =====
// 開始/一時停止はボタン名が変わり、モード切替は aria-pressed が変わるので支援技術に届く。
...
test('リセットは結果を支援技術へ伝える', async ({ page }) => {
  ...
  // control: 実際に動き出して残り時間が減っていること (減っていなければリセットを検証できない)
  await expect(page.locator('[role="timer"]')).not.toHaveText('25:00');

  await page.getByRole('button', { name: 'リセット', exact: true }).click();
  await expect(page.locator('#action-announcement')).toContainText('リセットしました');
  await expect(page.locator('#action-announcement')).toContainText('25:00');
  // 視覚トーストは出さない (頻繁な操作なので sr-only にとどめる — #1107 と同じ判断)。
  // container は最初の Toast.show で遅延生成されるので、**存在しないこと**が「1 度も出して
  // いない」の証明になる (存在する要素に対する否定より強い)。
