...
- **発信の状態**: 通常（`FROZEN.md` の POSTING-STATUS が `active`）。
- **自分で書いた不利な事実**: 247 件（`LICENSES/ACD-1.0.against.md`・不利なものを先に並べる）／**既知の欠陥**: 34 件（`ACD-1.0.errata.md`・全件未修正＝凍結中のため）。
